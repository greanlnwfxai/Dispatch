...
describe("HomePage (protected placeholder)", () => {
  ...
  it("shows a retry-safe offline message on a network failure, instead of redirecting to login", async () => {
  ...
  it("displays the safe authenticated principal after a successful bootstrap", async () => {
    const fetchMock = vi
      .fn()
      ...
      .mockResolvedValueOnce(
        jsonResponse({ userId: "u-1", displayName: "Jane Doe", roleCodes: ["INTERNAL_DELIVERY_EMPLOYEE"] }),
      );
    ...
    expect(screen.getByText("INTERNAL_DELIVERY_EMPLOYEE")).toBeTruthy();
  });
  ...
  it("never writes the access token to localStorage or sessionStorage", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(jsonResponse({ userId: "u-1", displayName: "Jane Doe", roleCodes: [] }));
    vi.stubGlobal("fetch", fetchMock);
