...
/**
 * Partial DRAFT edit (§7 — DRAFT only, explicit field allowlist, no mass
 * assignment). All fields optional: only fields explicitly present in the
 * request body are changed (see TasksService). Providing `destinationSource`
 * signals "reselect the destination" and then requires `searchId` plus the
 * full consistent field set for that source, exactly like create.
 */
export class UpdateDeliveryTaskDraftDto {
  @IsOptional()
  @IsUUID()
  searchId?: string;
