...
/**
 * Thin fetch wrapper for the AUTH-001 endpoints. The access token is
 * returned to the caller to hold in memory only (React state/ref) — this
 * module never reads or writes localStorage/sessionStorage/IndexedDB. The
 * refresh token is never handled here at all: the browser attaches/receives
 * it automatically via the HttpOnly `dispatch_refresh_token` cookie because
 * every call passes `credentials: "include"`.
 */
