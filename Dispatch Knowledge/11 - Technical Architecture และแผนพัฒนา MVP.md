...
| **AUTH-001** | Authentication และ RBAC | DEV-FOUNDATION-002, TDR-AUTH-001 (`APPROVED`) | JWT Access Token อายุสั้น + Rotating Refresh Token + Server-side Session/Revocation Store, Guard ชั้น 1–2 ตามหมวด 10 | ไม่มี | Permission matrix test (หมวด 19) ผ่านสำหรับ 6 Role | ไม่รวม business Command ของ MVP-02 ขึ้นไป |
...
| TDR-AUTH-001 | Auth/session strategy | Session-based, JWT stateless, JWT+rotating refresh+server-side revocation store | JWT Access Token อายุสั้น + Rotating Refresh Token + Server-side Session/Revocation Store | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้อง revoke ได้ทันที (account deactivation, Topic 03 §4.1) — ตอบสนองผ่าน server-side revocation record; ห้ามเก็บ token ใน `localStorage` | — (อนุมัติแนวทางแล้ว; **ยังไม่ implement** — AUTH-001 milestone เป็นผู้รับผิดชอบการ implement) |
