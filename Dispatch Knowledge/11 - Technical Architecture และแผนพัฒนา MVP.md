...
| **DEV-FOUNDATION-002** | Database และ API พื้นฐาน | DEV-FOUNDATION-001 (complete), TDR-DATABASE-001 (`APPROVED`), TDR-ORM-001 (`APPROVED`, direction only — schema/migrations begin in this milestone) | Schema เริ่มต้น (Identity, Role), API skeleton, health check | ไม่มี | Migration รันสำเร็จ, health check ผ่าน | ไม่มี business Command จริง |
...
| TDR-DATABASE-001 | Relational database | PostgreSQL, MySQL, SQL Server | PostgreSQL 16 (รองรับ JSONB สำหรับ evidence metadata ที่ยังเปลี่ยนแปลงได้) | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้องรองรับ relational integrity, append-only history, cross-record validation และ evidence metadata ที่ขยายได้ | — (อนุมัติแล้ว; provisioned ผ่าน Docker Compose ไม่มี host port mapping; ยังไม่มี schema/migration จนกว่าจะถึง DEV-FOUNDATION-002) |
| TDR-ORM-001 | ORM/data-access | Prisma, TypeORM, Kysely | Prisma + Repository pattern | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | Domain ต้องไม่ผูกกับ ORM โดยตรง (หลักการข้อ 6) | — (อนุมัติแนวทางแล้ว; Prisma schema/migration ธุรกิจยังไม่เริ่มจนกว่าจะถึง DEV-FOUNDATION-002) |
