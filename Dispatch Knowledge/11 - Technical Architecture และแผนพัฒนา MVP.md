...
> แถว "Local development" ด้านล่างคือข้อเสนอระดับแนวคิดสำหรับสถาปัตยกรรมเต็มรูปแบบในอนาคต (รวม MinIO เมื่อ TDR-STORAGE-001 ได้รับการอนุมัติ) สิ่งที่ implement จริงและได้รับการอนุมัติใน DEV-FOUNDATION-001 (ดู TDR-DEPLOY-001 หมวด 22) แคบกว่านี้: Docker Compose รัน **Postgres 16 เท่านั้น** (ไม่มี MinIO — evidence storage ยังเป็น `TECHNICAL_DECISION_REQUIRED`) + API + Admin Web + Mobile/PWA โดยทั้งสาม container รัน **production build** (`node dist/main.js`, `node server.js` จาก Next `output: "standalone"`) **ไม่ใช่ dev server** ตามที่ CLAUDE.md §11 กำหนด — พอร์ต: Admin Web 6001, API 6002, Mobile/PWA 6003, PostgreSQL ไม่มี host port mapping
