...
> [!summary]
> เอกสารฉบับนี้คือ **TECH-ARCH-001** ซึ่งแปลความรู้ทางธุรกิจที่ได้รับการอนุมัติแล้วใน Topics 01–10 ให้เป็นสถาปัตยกรรมทางเทคนิคที่พร้อมนำไปพัฒนาต่อ โดยไม่เปลี่ยนแปลง ขยาย หรือตัดสินใจนโยบายธุรกิจใด ๆ แทน Product Owner / User เอกสารนี้ **ไม่สร้างโค้ดแอปพลิเคชัน ไม่สร้าง Database Migration ไม่ติดตั้ง Dependency และไม่สร้าง Docker Container หรือ Deployment Configuration** สถานะเอกสาร: `DRAFT_FOR_REVIEW` — ณ วันที่จัดทำเอกสารฉบับร่างนี้ (2026-07-21) ทุกคำแนะนำด้านเทคโนโลยียังไม่ถือเป็นการอนุมัติ **อัปเดต 2026-07-22 (DEV-FOUNDATION-001A synchronization)**: Product Owner ได้อนุมัติ TDR-REPO-001, TDR-WEB-001, TDR-MOBILE-001, TDR-API-001, TDR-DATABASE-001, TDR-ORM-001 (แนวทาง), TDR-AUTH-001 (แนวทาง, ยังไม่ implement), TDR-TEST-001 และ TDR-CI-001 สำหรับขอบเขต DEV-FOUNDATION-001 แล้ว — ดูสถานะล่าสุดของแต่ละรายการใน Technical Decision Register หมวด 22 TDR-STORAGE-001, TDR-JOBS-001 และ production platform ของ TDR-DEPLOY-001 ยังคงไม่อนุมัติ
...
| Synchronization pass | **DEV-FOUNDATION-001A** (2026-07-22) — Technical Decision Register (หมวด 22) และหมวดที่เกี่ยวข้องถูกซิงค์ให้ตรงกับสถานะ TDR ที่ Product Owner อนุมัติจริงสำหรับ DEV-FOUNDATION-001 (ดู `CLAUDE.md` §2 และ `docs/CTO_SUMMARY_DEV_FOUNDATION_001.md`) เอกสารฉบับนี้ยังคงสถานะ `DRAFT_FOR_REVIEW` โดยรวม เนื่องจากยังมีรายการที่ยังไม่อนุมัติ (production deployment, TDR-STORAGE-001, TDR-JOBS-001) |
| Synchronization pass 2 | **DEV-FOUNDATION-002** (2026-07-22) — หมวด 7.1/7.2 (Identity and Access, User and Role) และหมวด 21 (Implementation Roadmap) ถูกซิงค์ให้ตรงกับ Prisma/PostgreSQL Identity/Role schema ที่ implement จริง (ดู `docs/CTO_SUMMARY_DEV_FOUNDATION_002.md`) การอนุมัติ User-to-Role cardinality **ไม่ได้ถูกตัดสินใจ**โดยงานนี้ — schema เป็นกลางทางเทคนิค (neutral) รองรับทั้งสองแนวทาง จนกว่า AUTH-001 หรือ Business Owner จะกำหนดนโยบาย |
| Synchronization pass 3 | **AUTH-001** (2026-07-22) — หมวด 10 (Authorization Architecture), หมวด 21 (Implementation Roadmap), และหมวด 22 (TDR-AUTH-001) ถูกซิงค์ให้ตรงกับ authentication/RBAC foundation ที่ implement จริง (ดู `docs/CTO_SUMMARY_AUTH_001.md`) User-to-Role cardinality ยังคงเป็นกลางทางเทคนิค — AUTH-001 อ่าน role assignment ทั้งหมดที่มีอยู่โดยไม่บังคับ single-role หรือ multi-role policy; นโยบายนั้นยังเป็นการตัดสินใจทางธุรกิจในอนาคต ไม่ใช่ผลจากงานนี้ |
| Synchronization pass 4 | **MVP-02** (2026-07-22) — หมวด 21 (Implementation Roadmap) ถูกซิงค์ให้ตรงกับ Customer Master search (read-only) และ Delivery Task creation/editing/submission ที่ implement จริง (ดู `docs/CTO_SUMMARY_MVP_02.md`) การซิงค์นี้เป็นการบันทึกการส่งมอบทางเทคนิคเท่านั้น **ไม่ได้อนุมัติ** BDR-TASK-001 (ชุดเอกสารอ้างอิงทางธุรกิจที่บังคับ) หรือ BDR-CUSTOMER-003 (ชุด field ของ Historical Destination Snapshot ที่ต้องตรึงไว้เกินกว่าค่าต่ำสุดที่อนุมัติแล้ว) ทั้งสองรายการยังคงเป็น OPEN Business Decision — งานนี้ implement เฉพาะค่าต่ำสุดที่อนุมัติแล้ว (ชื่อปลายทาง+ที่อยู่+Destination Source) บวก technical resolution ที่ยังปรับเปลี่ยนได้ในอนาคตโดยไม่กระทบ schema หลัก |
| Synchronization pass 5 | **MVP-02 remediation** (2026-07-23) — same-branch remediation after independent Codex review: UpdateDraft/SubmitDeliveryTask now serialize state-changing Task mutations with a PostgreSQL DeliveryTask row lock before fresh validation reads; concurrent submit can create exactly one status transition/status-history event; TaskEvent audit history now uses an additive `ON DELETE RESTRICT` FK migration (`20260723093000_task_event_delete_restrict`). Original applied MVP-02 migration remains unchanged. Remote CI remains NOT YET RUN. |
| Synchronization pass 6 | **MVP-03** (2026-07-23) — หมวด 21 (Implementation Roadmap) และหมวด 22 (TDR-STORAGE-001 implementation status) ถูกซิงค์ให้ตรงกับ Preparation และ Pre-loading Evidence ที่ implement จริง (ดู `docs/CTO_SUMMARY_MVP_03.md`) การซิงค์นี้เป็นการบันทึกการส่งมอบทางเทคนิคเท่านั้น **ไม่ได้อนุมัติ** BDR-PREP-002, BDR-PREP-003, evidence content/quality policy, long-term retention policy, หรือ Assignment/DeliveryAttempt linkage policy; รายการเหล่านี้ยังคง OPEN Business Decision |

**Scope statement**: เอกสารนี้กำหนดสถาปัตยกรรมทางเทคนิคระดับ logical/component สำหรับ Dispatch MVP ครอบคลุม System Context, Application Architecture, Technology Stack Evaluation (ณ ร่างต้นฉบับ 2026-07-21 ยังไม่อนุมัติ — ดูสถานะปัจจุบันหลัง DEV-FOUNDATION-001A synchronization ในหมวด 22), Repository Structure ที่เสนอ, Domain/Module Map, Aggregate Boundaries, Status Architecture, Authorization Architecture, Validation Architecture, Evidence Architecture, Audit/Timeline Architecture, Privacy Architecture, Returned-Goods Architecture, Emergency Override Architecture, API/Command Boundary, Error Model, Testing Architecture, Deployment Architecture, Implementation Roadmap, Technical Decision Register และ Open Business Decision Boundary
...
- อนุมัติ Technology Stack ใด ๆ ด้วยตัวเอกสารนี้เอง — ณ ร่างต้นฉบับ (2026-07-21) ทุกข้อเสนอในหมวด 5 และ 22 มีสถานะ `RECOMMENDED_FOR_APPROVAL`, `TECHNICAL_DECISION_REQUIRED`, หรือ `DEFERRED` เท่านั้น (ดูหมายเหตุ synchronization ในหมวด 0 และสถานะปัจจุบันในหมวด 22 สำหรับรายการที่ Product Owner อนุมัติแล้วภายหลังสำหรับขอบเขต DEV-FOUNDATION-001)
...
หมวด 5 (Technology Stack Evaluation) และหมวด 22 (Technical Decision Register) เดิมระบุว่าตัวเลือกเทคโนโลยีทั้งหมด**ยังไม่ได้รับการอนุมัติ** ก่อนเริ่ม DEV-FOUNDATION-001 **อัปเดตหลัง DEV-FOUNDATION-001A synchronization (2026-07-22)**: Product Owner ได้อนุมัติ TDR-REPO-001, TDR-WEB-001, TDR-MOBILE-001, TDR-API-001, TDR-DATABASE-001, TDR-ORM-001 (แนวทาง), TDR-AUTH-001 (แนวทาง), TDR-TEST-001 และ TDR-CI-001 ตามขอบเขตของ DEV-FOUNDATION-001 แล้ว ส่วนที่ยังรอการตัดสินใจทางเทคนิคคือ TDR-STORAGE-001, TDR-JOBS-001 และ production platform/orchestration/topology ของ TDR-DEPLOY-001 (local Docker Compose ได้รับการอนุมัติแล้ว แต่ production ยังไม่อนุมัติ) — ดูหมวด 22 สำหรับสถานะโดยละเอียด
...
> [!warning] สถานะการอนุมัติ (ปรับปรุง 2026-07-22)
> หมวดนี้เดิมเป็นการวิเคราะห์ล้วน ๆ ก่อนเริ่ม DEV-FOUNDATION-001 (ไม่มีรายการใดเป็น `APPROVED`) หลัง DEV-FOUNDATION-001A synchronization Product Owner ได้อนุมัติ TDR-REPO-001, TDR-WEB-001, TDR-MOBILE-001, TDR-API-001, TDR-DATABASE-001, TDR-ORM-001 (แนวทาง), TDR-AUTH-001 (แนวทาง), TDR-TEST-001 และ TDR-CI-001 ตามขอบเขตที่ implement จริงใน DEV-FOUNDATION-001 แล้ว — สถานะ "Decision status" ของแต่ละหัวข้อด้านล่างสะท้อนสถานะล่าสุด ส่วนที่ยังไม่อนุมัติคือ TDR-STORAGE-001, TDR-JOBS-001 และ production platform ของ TDR-DEPLOY-001 (ดู Technical Decision Register หมวด 22 เป็น authoritative source)
...
| **Decision status** | `APPROVED` (TDR-REPO-001 — Product Owner, DEV-FOUNDATION-001) | — |
...
**Decision status**: `APPROVED` (TDR-WEB-001 — Product Owner, DEV-FOUNDATION-001)
...
**Decision status**: `APPROVED` (TDR-MOBILE-001 — Product Owner, DEV-FOUNDATION-001)
...
**Decision status**: `APPROVED` (TDR-API-001 — Product Owner, DEV-FOUNDATION-001)
...
**Decision status**: `APPROVED` (TDR-ORM-001 — Product Owner, direction approved for DEV-FOUNDATION-001; Prisma business schema/migrations remain **out of scope until DEV-FOUNDATION-002**)
...
**Decision status**: `APPROVED` (TDR-DATABASE-001 — Product Owner, DEV-FOUNDATION-001; PostgreSQL 16 provisioned via Docker Compose, not yet connected to by the API)

### 5.7 Authentication / Session Strategy

> [!important] อัปเดต 2026-07-22 (DEV-FOUNDATION-001A synchronization)
> แนวทางที่ Product Owner อนุมัติคือ **JWT Access Token อายุสั้น + Rotating Refresh Token + Server-side Session/Revocation Store** ซึ่ง**แทนที่ (supersedes)** คำแนะนำ Session-based + Redis เดิมของหมวดนี้ ตารางเปรียบเทียบด้านล่างคงไว้เป็นหลักฐานการวิเคราะห์ดั้งเดิม แต่ Recommendation/Decision status สะท้อนแนวทางที่อนุมัติจริง
...
| JWT (stateless access token + refresh token เท่านั้น ไม่มี server-side record) | Scale ง่ายกว่าในทางทฤษฎี, ไม่ต้องมี session store | Revoke ทันทียากกว่า (ต้องมี blacklist/short expiry), เพิ่มความซับซ้อนสำหรับ requirement "ปิดบัญชีทันที" |
| **JWT Access Token อายุสั้น + Rotating Refresh Token + Server-side Session/Revocation Store (แนวทางที่อนุมัติ)** | รวมข้อดีของทั้งสองแบบ: short-lived access token ลดความเสี่ยงหาก token รั่วไหล, rotating refresh token ลด replay risk, server-side revocation record ให้ revoke ได้ทันทีเทียบเท่า session-based (ตรงกับ requirement "ปิดบัญชีทันที" ใน Topic 03 §4.1) | ซับซ้อนกว่า JWT ล้วนหรือ session ล้วน — ต้องดูแลทั้ง token rotation logic และ revocation store |

**ผลกระทบด้าน Operation**: Server-side session/revocation store ให้ revoke ทันที (account deactivation) เทียบเท่ากับแนวทาง session-based เดิม โดยไม่ต้องพึ่ง blacklist แบบ JWT ล้วน
**ผลกระทบด้าน Development**: Access token อายุสั้นลดพื้นผิวการโจมตีหาก token รั่วไหล ต้องไม่เก็บ token ใน `localStorage` (ดูหมายเหตุด้านล่าง)
**Recommendation**: JWT Access Token อายุสั้น + Rotating Refresh Token + Server-side Session/Revocation Store พร้อม RBAC guard ตามหมวด 10
**Decision status**: `APPROVED` (TDR-AUTH-001 — Product Owner-authorized direction; **supersedes Topic 11 §5.7's original session-based recommendation** ตาม explicit Product Owner authorization ที่บันทึกใน `CLAUDE.md` §2)

> [!warning] แนวทางนี้เป็นการอนุมัติสถาปัตยกรรม ไม่ใช่การ implement
> - แนวทางนี้**ยังไม่ถูก implement** ใน DEV-FOUNDATION-001 — ไม่มีโค้ด login, JWT, guard, หรือ session/revocation store ใด ๆ ในงานนี้
> - Milestone **AUTH-001** เป็นผู้รับผิดชอบการ implement แนวทางนี้
> - Access token **ต้องไม่ถูกเก็บใน `localStorage`** ภายใต้สถานการณ์ใด ๆ
> - การ revoke ทันที/ปิดบัญชีทันที (account deactivation) จะพึ่งพา server-side session/revocation record เป็นกลไกหลัก
...
**Decision status**: `APPROVED` (TDR-API-001 เดียวกับ Backend Framework — รวมการตัดสินใจ; Product Owner, DEV-FOUNDATION-001)

### 5.11 Testing Framework

| Candidate | ข้อดี | ข้อเสีย |
| --- | --- | --- |
| Jest/Vitest (unit + integration) + Supertest (API) + Playwright (E2E/PWA) | ครอบคลุมทุกระดับที่ระบุในหมวด 19, ecosystem TypeScript เดียวกับ stack ที่เสนอ | ต้อง setup หลายเครื่องมือร่วมกัน |
| Mocha/Chai | ยืดหยุ่นเช่นกัน | Ecosystem รอบ TypeScript/monorepo ไม่แข็งแรงเท่า Vitest ในปัจจุบัน |

**Recommendation (อัปเดต 2026-07-22 ตามที่ implement จริงและตรวจสอบแล้วใน DEV-FOUNDATION-001)**: **Jest** (NestJS native, ผ่าน `ts-jest`) สำหรับ `apps/api` — unit spec (`*.spec.ts`) และ `test/*.e2e-spec.ts` ผ่าน `jest --config ./test/jest-e2e.json`; **Vitest** สำหรับ `packages/*` (shared-types, domain, validation, contracts, test-utils) และ `apps/admin-web`/`apps/mobile-pwa`; **Supertest** สำหรับ API integration ภายใน Jest e2e spec ของ `apps/api`; **Playwright** สำหรับ E2E ระดับ workspace (`e2e/`) ไม่ใช่ Vitest ล้วนตามที่วิเคราะห์ไว้เดิม — Jest ถูกเลือกสำหรับ `apps/api` เพราะเป็น test runner ที่ NestJS สร้างให้โดย native scaffolding (`@nestjs/testing`)
**Decision status**: `APPROVED` (TDR-TEST-001 — Product Owner, DEV-FOUNDATION-001; ดูรายละเอียด test runner ต่อ workspace ในหมวด 22)
...
**Decision status**: `TECHNICAL_DECISION_REQUIRED` (TDR-DEPLOY-001) — **แยกขอบเขตชัดเจน (อัปเดต 2026-07-22)**:
- **อนุมัติแล้ว (local development)**: macOS + Docker Desktop + Docker Compose; Admin Web host port 6001, Backend API host port 6002 (`GET /health` ที่ `/health`), Mobile/PWA host port 6003, PostgreSQL internal Docker port 5432 โดยไม่มี host port mapping — ตามที่ implement จริงใน `docker-compose.yml` ของ DEV-FOUNDATION-001
- **ยังไม่อนุมัติ/ยังเปิดอยู่ (production)**: Production hosting platform, production orchestration/platform selection, และ production infrastructure topology ยังคงเป็น `TECHNICAL_DECISION_REQUIRED` — เอกสารนี้**ไม่ทำเครื่องหมาย TDR-DEPLOY-001 เป็น `APPROVED` เต็มรูปแบบ** เนื่องจากส่วน production ยังไม่ได้รับการตัดสินใจ
...
**Decision status**: `APPROVED` (TDR-CI-001 — Product Owner, DEV-FOUNDATION-001)
...
> [!important] อัปเดต 2026-07-22 — สถานะจริงของ Local development หลัง DEV-FOUNDATION-001
> แถว "Local development" ด้านล่างคือข้อเสนอระดับแนวคิดสำหรับสถาปัตยกรรมเต็มรูปแบบในอนาคต (รวม MinIO เมื่อ production-grade TDR-STORAGE-001 adapter ถูกเลือก) สิ่งที่ implement จริงและได้รับการอนุมัติใน DEV-FOUNDATION-001 (ดู TDR-DEPLOY-001 หมวด 22) แคบกว่านี้: Docker Compose รัน **Postgres 16** + API + Admin Web + Mobile/PWA โดยทั้งสาม container รัน **production build** (`node dist/main.js`, `node server.js` จาก Next `output: "standalone"`) **ไม่ใช่ dev server** ตามที่ CLAUDE.md §11 กำหนด — พอร์ต: Admin Web 6001, API 6002, Mobile/PWA 6003, PostgreSQL ไม่มี host port mapping; อัปเดต MVP-03 (2026-07-23): evidence storage ใช้ filesystem-backed development adapter บน Docker named volume `dispatch_evidence_data` ผ่าน API เท่านั้น ไม่มี public bucket/host port และ production target ยังเป็น S3-compatible private object storage
...
| **DEV-FOUNDATION-001** ✅ `COMPLETE` (2026-07-22) | Repository และ Tooling พื้นฐาน | TECH-ARCH-001; TDR-REPO-001, TDR-WEB-001, TDR-MOBILE-001, TDR-API-001, TDR-DATABASE-001, TDR-ORM-001 (แนวทาง), TDR-AUTH-001 (แนวทาง), TDR-TEST-001, TDR-CI-001, TDR-DEPLOY-001 (local only) — ทั้งหมด `APPROVED` โดย Product Owner | Repository structure จริงตามหมวด 6, lint/typecheck config, CI skeleton, NestJS `GET /health` skeleton, สอง Next.js foundation-status app, PostgreSQL ผ่าน Docker Compose (ดู `docs/CTO_SUMMARY_DEV_FOUNDATION_001.md`) | ไม่มี | Build/lint/typecheck ผ่าน | ไม่มี business logic ใด ๆ |
| **DEV-FOUNDATION-002** ✅ `COMPLETE` (2026-07-22) | Database และ API พื้นฐาน | DEV-FOUNDATION-001 (complete), TDR-DATABASE-001 (`APPROVED`), TDR-ORM-001 (`APPROVED`, direction only — schema/migrations begin in this milestone) | Identity/Role schema (`User`, `Role`, `UserRoleAssignment` — technical foundation only, no default User), initial migration, idempotent system-role seed (6 roles), database-aware `/health`, `/health/live`, `/health/ready`, Identity/Role repository boundary (ดู `docs/CTO_SUMMARY_DEV_FOUNDATION_002.md`) | ไม่มี | Migration รันสำเร็จ, health check ผ่าน | ไม่มี business Command จริง; ไม่มี login/JWT/password; User-to-Role cardinality ยังเป็น neutral technical boundary รอ AUTH-001 |
| **AUTH-001** ✅ `COMPLETE` (2026-07-22) | Authentication และ RBAC | DEV-FOUNDATION-002 (complete), TDR-AUTH-001 (`APPROVED`) | JWT Access Token อายุสั้น (in-memory เท่านั้น) + Rotating Opaque Refresh Token (HttpOnly cookie เท่านั้น, ไม่เคยอยู่ใน JSON) + Server-side Session/Revocation Store (`AuthSession`, `RefreshTokenRecord`), `JwtAuthenticationGuard` (global) + `RolesGuard` ตามหมวด 10, neutral `loginId` (trim+lowercase, ไม่ใช่ email), operator-only SUPER_ADMIN bootstrap CLI (ไม่มี default account), login/session shell สำหรับ Admin Web และ Mobile/PWA (ดู `docs/CTO_SUMMARY_AUTH_001.md`) | ไม่มี | Unit/integration/e2e ผ่านทั้งหมด (login, refresh rotation, reuse detection, revocation, RBAC 401/403 สำหรับ 6 Role) | ไม่รวม business Command ของ MVP-02 ขึ้นไป; ไม่รวม User/Role-management UI; ไม่รวม password reset/MFA/SSO; production secret/cookie-domain configuration ยังไม่กำหนด |
| **MVP-02** ✅ `ENGINEERING COMPLETE` (2026-07-22; remediation 2026-07-23) | Customer และ Task Creation | AUTH-001 (complete) | CreateDeliveryTask/UpdateDeliveryTaskDraft/SubmitDeliveryTask, Customer Master search (read-only, search-first evidence via `CustomerMasterSearch`), Historical Destination Snapshot (approved minimum: destination name + address + Destination Source), flexible optional `TaskReference`; remediation adds row-lock serialization for DRAFT edits/submits and TaskEvent `ON DELETE RESTRICT` audit protection (ดู `docs/CTO_SUMMARY_MVP_02.md`) | **ยังคง OPEN**: BDR-TASK-001 (reference number set) — ไม่มี reference type ใดถูกบังคับ; BDR-CUSTOMER-003 (frozen snapshot fields เกินกว่าค่าต่ำสุด) — งานนี้ implement เฉพาะค่าต่ำสุดที่อนุมัติแล้วบวก technical resolution ที่ปรับเปลี่ยนได้ ทั้งสองรายการไม่ได้ถูกอนุมัติโดยงานนี้ | Unit/integration/e2e/Playwright ผ่านทั้งหมด (search-first evidence, MASTER/FREE_TEXT consistency, snapshot immutability, task-number concurrency, row-lock submit/edit concurrency, submit atomicity, TaskEvent restrictive FK, RBAC 401/403) | ไม่รวม Preparation/Assignment/Delivery/Return/Reopen/Override/Reporting; ไม่รวม Customer Master CRUD |
| **MVP-03** ✅ `ENGINEERING COMPLETE` (2026-07-23) | Preparation และ Pre-loading Evidence | MVP-02 | StartPreparation, UpdatePreparation, PreparationIssue, private Pre-loading Evidence, ConfirmPreparationReady, Stock discrepancy report, Admin PreparationCorrectionRecord, Super Admin retrospective review, Admin Web preparation UI + review queue (ดู `docs/CTO_SUMMARY_MVP_03.md`) | **ยังคง OPEN**: BDR-PREP-002, BDR-PREP-003, evidence content/quality policy, exact long-term retention policy, Assignment/DeliveryAttempt linkage policy — งานนี้ไม่สร้าง Assignment/DeliveryAttempt และไม่ตัดสินใจ equality/retry/reopen policy | Unit/integration/e2e/Playwright ผ่านทั้งหมด (photo gate, unresolved issue gate, private evidence retrieval, correction governance, zero residue) | ไม่รวม Assignment, DeliveryAttempt, Delivery Start, GPS, handover, recipient/signature, delivered quantity, partial/failed delivery, returned goods, reopen, Emergency Override, notifications, reporting/export |
| **MVP-04** ✅ `ENGINEERING COMPLETE` (2026-07-23) | Assignment | MVP-03 | AssignDeliveryTask (`READY_FOR_DISPATCH -> ASSIGNED`), formal reassignment (`ASSIGNED -> ASSIGNED`, mandatory reason, stale-write precondition), append-only assignment history, `GET /assignment-candidates` (active-workload visibility, non-blocking), Internal Delivery Employee's own record-scoped `GET /assigned-tasks` read-only view, Admin Web assignment/reassignment UI, Mobile/PWA "My assigned tasks" (ดู `docs/CTO_SUMMARY_MVP_04.md`) | BDR-ASSIGN-001 to 005 — approved (see `docs/CTO_SUMMARY_MVP_04.md` §4; the formal Topic 07 register annotation remains a separate Product-Owner/business action, per CLAUDE.md §19 "Topics 01–10 are never modified by an engineering task") | Unit/integration/e2e/concurrency/Playwright ผ่านทั้งหมด (RBAC 401/403/404, record-scope 404 for supporting-only/unrelated employee, initial-assignment and reassignment validation, database-level one-current-assignment backstop, stale-reassignment rejection with zero residue) | ไม่รวม Delivery Start, DeliveryAttempt, GPS check-in, handover evidence, recipient/signature, Return, Reopen, Emergency Override |
...
> [!warning] สถานะการอนุมัติ (ปรับปรุง 2026-07-22 — DEV-FOUNDATION-001A synchronization)
> ตารางนี้เดิมไม่มีรายการใดเป็น `APPROVED` (ทุกสถานะเป็น `RECOMMENDED_FOR_APPROVAL`, `TECHNICAL_DECISION_REQUIRED`, หรือ `DEFERRED` เท่านั้น) หลังจาก Product Owner อนุมัติขอบเขต DEV-FOUNDATION-001 และงาน implement เสร็จสิ้น (ดู `CLAUDE.md` §2 และ `docs/CTO_SUMMARY_DEV_FOUNDATION_001.md`) คอลัมน์ "Status" ด้านล่างถูกซิงค์ให้ตรงกับการอนุมัติจริง — สถานะที่เป็น `APPROVED` มาจาก Product Owner / User โดยตรงเท่านั้น ไม่ใช่การตัดสินใจของ Technical Architecture เอง TDR-STORAGE-001 และ TDR-JOBS-001 ยังคงสถานะเดิม (ไม่ถูกแตะต้อง) TDR-DEPLOY-001 มีสถานะผสม — ดูรายละเอียดในแถวของตนเองและหมวด 5.12
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TDR-REPO-001 | Monorepo หรือ separated repos | Monorepo, Separated | Monorepo | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ไม่มี | — (อนุมัติแล้ว) |
| TDR-WEB-001 | Admin Web framework | React/Next.js, Vue/Nuxt, Svelte | React + Next.js | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้องรองรับ Permission Matrix UI ที่ซับซ้อน | — (อนุมัติแล้ว) |
| TDR-MOBILE-001 | Mobile/PWA framework | React PWA, React Native, Flutter | React PWA | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้องรองรับ GPS + Camera โดยไม่ต้องผ่าน App Store review รอบแรก | — (อนุมัติแล้ว) |
| TDR-API-001 | Backend framework + API style | NestJS/Express/.NET/FastAPI; REST/GraphQL | NestJS + REST command-oriented | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้องรองรับ layered architecture หมวด 4 | — (อนุมัติแล้ว) |
| TDR-DATABASE-001 | Relational database | PostgreSQL, MySQL, SQL Server | PostgreSQL 16 (รองรับ JSONB สำหรับ evidence metadata ที่ยังเปลี่ยนแปลงได้) | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้องรองรับ relational integrity, append-only history, cross-record validation และ evidence metadata ที่ขยายได้ | — (อนุมัติแล้ว; provisioned ผ่าน Docker Compose ไม่มี host port mapping; Identity/Role schema/migration แรก implement แล้วใน DEV-FOUNDATION-002 — business aggregate schema ยังเป็นงานในอนาคต) |
| TDR-ORM-001 | ORM/data-access | Prisma, TypeORM, Kysely | Prisma + Repository pattern | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | Domain ต้องไม่ผูกกับ ORM โดยตรง (หลักการข้อ 6) | — (อนุมัติแนวทางแล้ว; Prisma schema/migration สำหรับ Identity/Role technical foundation implement แล้วใน DEV-FOUNDATION-002 ผ่าน Repository Pattern boundary ใน `packages/domain`; business aggregate schema ยังไม่เริ่ม) |
| TDR-AUTH-001 | Auth/session strategy | Session-based, JWT stateless, JWT+rotating refresh+server-side revocation store | JWT Access Token อายุสั้น + Rotating Refresh Token + Server-side Session/Revocation Store | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; **implemented** AUTH-001, 2026-07-22 | ต้อง revoke ได้ทันที (account deactivation, Topic 03 §4.1) — ตอบสนองผ่าน server-side revocation record (`AuthSession.revokedAt`); ห้ามเก็บ token ใน `localStorage` | — (implement แล้วเต็มรูปแบบใน AUTH-001: `POST /auth/login`/`refresh`/`logout`/`logout-all`, `GET /auth/me`, `JwtAuthenticationGuard` + `RolesGuard`, ดู `docs/CTO_SUMMARY_AUTH_001.md`) |
| TDR-STORAGE-001 | Evidence object storage | S3-compatible, DB BLOB, filesystem | S3-compatible private object storage remains production target; MVP-03 uses filesystem-backed development adapter on Docker named volume to avoid committing or printing object-storage credentials | `IMPLEMENTED_FOR_MVP_03_DEV_ADAPTER` (technical implementation decision, not Product Owner business approval) | Technical Architecture | MVP-03 implementation, 2026-07-23 | ต้องรองรับ retention policy ที่ยังเปิดอยู่หลายรายการ; evidence remains private and API-authorized | Production S3-compatible adapter, retention automation, and deployment topology remain future technical work |
...
| TDR-TEST-001 | Testing framework | Vitest/Jest, Mocha | **Jest** (NestJS native + `ts-jest`) สำหรับ `apps/api`; **Vitest** สำหรับ `packages/*`, `apps/admin-web`, `apps/mobile-pwa`; **Supertest** สำหรับ API integration; **Playwright** สำหรับ E2E | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ต้องรองรับ traceability test ตามหมวด 11.3 | — (อนุมัติแล้ว; ยืนยัน exact test runner ต่อ workspace จากการตรวจสอบ repository จริง — ไม่ใช่ Vitest ล้วนตามที่วิเคราะห์ไว้ในหมวด 5.11 เดิม) |
| TDR-DEPLOY-001 | Containerization/deployment | Docker+managed platform, PaaS โดยตรง | Docker (local/staging/CI) + managed platform (prod, ยังไม่เลือก) | `TECHNICAL_DECISION_REQUIRED` (ผสม — ดูรายละเอียด) | Product Owner (local); Technical Architecture (production, ยังเปิดอยู่) | Local: DEV-FOUNDATION-001 (2026-07-22, อนุมัติแล้ว); Production: ก่อน staging environment แรก (ยังไม่กำหนด) | ต้องแยก environment ตามหมวด 20 | **Local development อนุมัติแล้ว**: macOS + Docker Desktop + Docker Compose, Admin Web 6001, API 6002, Mobile/PWA 6003, PostgreSQL internal-only (ไม่มี host port mapping) **Production ยังไม่อนุมัติ**: production hosting platform, production orchestration/platform selection, production infrastructure topology ยังเป็น `TECHNICAL_DECISION_REQUIRED` — ล่าช้าในการตั้ง staging/production |
| TDR-CI-001 | CI approach | GitHub Actions, อื่น ๆ | GitHub Actions | `APPROVED` | Product Owner | approved for DEV-FOUNDATION-001; recorded 2026-07-22 | ไม่มี | — (อนุมัติแล้ว; `.github/workflows/ci.yml` รัน lint/typecheck/test/build ทุก workspace; Docker build/E2E ยังเป็น local gate ไม่ใช่ CI job ตามขอบเขตนี้) |
...
- [x] ณ ร่างต้นฉบับของเอกสารนี้ (2026-07-21, ก่อน DEV-FOUNDATION-001) ข้อเสนอทางเทคนิคทั้งหมดถูกทำเครื่องหมายว่ายังไม่อนุมัติ (`RECOMMENDED_FOR_APPROVAL` / `TECHNICAL_DECISION_REQUIRED` / `DEFERRED` เท่านั้น — ไม่มี `APPROVED`) — **สถานะปัจจุบัน (หลัง DEV-FOUNDATION-001A synchronization, 2026-07-22) อยู่ในหมวด 22**: 9 รายการได้รับการอนุมัติจาก Product Owner ตามขอบเขต DEV-FOUNDATION-001 (TDR-REPO-001, TDR-WEB-001, TDR-MOBILE-001, TDR-API-001, TDR-DATABASE-001, TDR-ORM-001, TDR-AUTH-001, TDR-TEST-001, TDR-CI-001) TDR-STORAGE-001 และ TDR-JOBS-001 ยังไม่อนุมัติ; TDR-DEPLOY-001 อนุมัติเฉพาะส่วน local development เท่านั้น
...
## 26. Architecture Decision Records (ADR) Cross-Reference

> [!note] เพิ่มเข้ามาเมื่อ 2026-07-24 (ADR Foundation) — ไม่แก้ไขเนื้อหาเดิม
> หมวดนี้เป็นการเพิ่ม cross-reference เท่านั้น ไม่เปลี่ยนแปลงเนื้อหาสถาปัตยกรรม
> เดิมในหมวด 1–25 และไม่ปิด Open Business Decision ใด ๆ ที่ยังเปิดอยู่

นับตั้งแต่ 2026-07-24 เป็นต้นไป Dispatch มี Architecture Decision Record
(ADR) ชุดแรกที่ `docs/adr/` ซึ่งบันทึกว่า **ทำไม**สถาปัตยกรรมทางเทคนิคที่
implement จริงถึง MVP-04 จึงถูกเลือกแบบนี้ และอะไรคือสิ่งที่ implementation
ในอนาคตต้องรักษาไว้ — เอกสารนี้ (Topic 11) ยังคงเป็น authoritative source
ของ**สถาปัตยกรรมและแผนพัฒนา MVP โดยรวม** ส่วน ADR เป็นบันทึกการตัดสินใจ
รายประเด็นที่ traceable กลับมาที่เอกสารนี้ ไม่ใช่เอกสารที่แทนที่กัน

**ความแตกต่างของเอกสารกำกับดูแลแต่ละประเภท**:

| ประเภท | ตอบคำถาม | อำนาจ |
| --- | --- | --- |
| BDR (Topic 07) | ธุรกิจตัดสินใจอะไร | Product Owner / User เท่านั้น |
| TDR (หมวด 22 ของเอกสารนี้) | ตัวเลือกทางเทคนิคที่ยังไม่ตกผลึกเป็นสถาปัตยกรรม | Product Owner (อนุมัติ) / Technical Architecture (วิเคราะห์) |
| ADR (`docs/adr/`) | ทำไมสถาปัตยกรรมทางเทคนิคจึงถูกเลือกแบบนี้ | Architecture / Engineering ภายใต้ขอบเขตที่ BDR อนุมัติแล้ว |
| CTO Summary (`docs/CTO_SUMMARY_*.md`) | สิ่งที่ถูก implement จริงต่อ milestone | Engineering (บันทึกการส่งมอบ ไม่ใช่การอนุมัติ) |

**ADR ชุดแรก (backfill ย้อนหลังจากสถาปัตยกรรมที่ implement จริงถึง MVP-04)**:
...
รายละเอียดเต็มของแต่ละ ADR (context, considered options, consequences,
repository evidence) อยู่ที่ `docs/adr/` เท่านั้น — หมวดนี้ไม่ทำซ้ำเนื้อหา
ดังกล่าว ดูดัชนีและนโยบายฉบับเต็มที่ [`docs/adr/README.md`](../docs/adr/README.md)

ADR ไม่เปลี่ยนสถานะของ BDR หรือ TDR ใด ๆ — BDR-RETURN-007, BDR-RETURN-009,
TDR-STORAGE-001 (production provider), TDR-JOBS-001, และ TDR-DEPLOY-001
(production) ยังคงสถานะเปิดอยู่ตามที่บันทึกไว้ในหมวด 22 และ 23 ของเอกสารนี้
เช่นเดิม
