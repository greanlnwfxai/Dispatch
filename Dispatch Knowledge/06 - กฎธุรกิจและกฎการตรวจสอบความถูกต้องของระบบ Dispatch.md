...
> [!summary]
> เอกสารฉบับนี้รวบรวมและจัดระบบกฎธุรกิจ (Business Rule) และกฎการตรวจสอบความถูกต้อง (Validation Rule) ที่ได้รับอนุมัติแล้วจาก [[01 - เป้าหมายของระบบ Dispatch]], [[02 - Workflow การทำงานของระบบ Dispatch]], [[03 - บทบาทและสิทธิ์ผู้ใช้งาน]] และ [[04 - สถานะของงานและกติกาการเปลี่ยนสถานะ]] รวมถึง [[05 - ข้อมูล หลักฐาน และรายละเอียดที่ต้องจัดเก็บในแต่ละงาน]] ให้เป็น Catalog เดียวที่สอดคล้องกัน (Consistent Rule Catalog) เอกสารฉบับนี้กำหนดลำดับความสำคัญของกฎ, นิยาม Business Rule แยกจาก Validation Rule, ระดับการบังคับใช้, ช่วงเวลาตรวจสอบ, Business Rule and Validation Matrix มากกว่า 100 รายการ, Checklist เฉพาะการกระทำอย่างน้อย 30 ชุด, ตัวอย่างสถานการณ์อย่างน้อย 24 กรณี และรายการ Open Business Decisions ที่ยังไม่ได้รับการอนุมัติ เอกสารฉบับนี้เป็นเอกสารระดับธุรกิจเท่านั้น **ไม่ใช่**เอกสารออกแบบฐานข้อมูล API หน้าจอ หรือ Test Case

เอกสารฉบับนี้ต่อยอดจาก Topics 1–5 ทั้งหมด เนื้อหาทั้งหมดต้องสอดคล้องกับการตัดสินใจทางธุรกิจที่ได้รับการอนุมัติแล้ว **ไม่แก้ไขหรือขัดแย้งกับ Topics 1–5** และจะไม่ตัดสินใจแทนเจ้าของธุรกิจในประเด็นที่ยังไม่ชัดเจน ประเด็นเหล่านั้นถูกรวบรวมไว้ในหัวข้อ [40. Open Business Decisions](#40-open-business-decisions)
...
เอกสารฉบับนี้ไม่สร้างกฎใหม่ด้วยตนเอง ทุกกฎในเอกสารฉบับนี้ต้องตรวจสอบย้อนกลับไปยังหนึ่งใน Topics 1–5 ได้เสมอ ยกเว้นกรณีที่ระบุไว้อย่างชัดเจนว่าเป็น "ข้อเสนอ" (Proposal) ซึ่งจะไม่ปรากฏในเอกสารฉบับนี้เว้นแต่จำเป็นต้องรวบรวมไว้เป็นประเด็นเปิดในหัวข้อ [40](#40-open-business-decisions)
...
| 10 | Open Business Decisions | ประเด็นนี้เป็นเรื่องที่ยังไม่ได้รับอนุมัติหรือไม่ |
...
* **BR-STATE-004** — ประเด็นทางธุรกิจที่ยังไม่ได้รับการยืนยันต้องไม่ถูก Implement ราวกับว่าได้รับการอนุมัติแล้ว (An unresolved business decision must not be implemented as though it were approved) — ดูหัวข้อ [40](#40-open-business-decisions)
...
| BR-TASK-003 | ข้อมูลปลายทางต้องเพียงพอต่อการปฏิบัติงาน (ชื่อปลายทาง, ที่อยู่) | HARD_BLOCK | Topic 2 §5.2; Topic 5 §6 |
| BR-TASK-004 | วันที่วางแผนจัดส่งต้องถูกบันทึกสำหรับ Task ปัจจุบันที่เกี่ยวข้อง | HARD_BLOCK | Topic 5 §6 |
| BR-TASK-005 | เลขที่เอกสารอ้างอิงทางธุรกิจต้องถูกบันทึกเมื่อเกี่ยวข้อง แต่ชุดที่บังคับยังไม่ได้ข้อสรุป | CONDITIONAL / ADVISORY | Topic 5 §6 (เปิด — ดู §40) |
...
| BR-TASK-009 | คำแนะนำปลายทางในอดีตที่เคยใช้จริงต้องไม่ถูกแทนที่แบบเงียบด้วยข้อมูลหลักที่เปลี่ยนแปลงภายหลัง (Historical Destination Snapshot) | HARD_BLOCK | Topic 5 §8 |
...
* **VR-TASK-001a** — ก่อนบันทึกงานเป็น WAITING_PREPARATION ตรวจสอบว่าชื่อลูกค้า/ปลายทาง ที่อยู่ และรายการสินค้ายืนยันครบถ้วนแล้ว — Timing: BEFORE_PREPARATION — อ้างอิง BR-TASK-008
* **VR-TASK-002a** — ก่อนเริ่มการจัดส่งของ Attempt ใด ตรวจสอบว่า Snapshot ปลายทางที่ใช้จริงถูกตรึงไว้แยกจากข้อมูลหลักปัจจุบัน — Timing: BEFORE_DELIVERY_START — อ้างอิง BR-TASK-009
...
| BR-PREP-007 | การแก้ไขข้อมูลการเตรียมสินค้าหลัง IN_TRANSIT ต้องผ่าน Correction/Exception Record ที่ดำเนินการโดย Admin หรือ Super Admin เท่านั้น | CORRECTION_REQUIRED | Topic 3 §14.2; Topic 4 §27 |
| BR-PREP-008 | อำนาจที่แน่นอนของการแก้ไขข้อมูลการเตรียมสินค้าหลัง IN_TRANSIT (Admin เองหรือจำกัดเฉพาะ Super Admin) ยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
* **VR-PREP-003a** — เมื่อ Stock รายงานความไม่ตรงกันหลัง IN_TRANSIT ตรวจสอบว่าผู้ดำเนินการแก้ไขคือ Admin หรือ Super Admin เท่านั้น — Timing: BEFORE_CORRECTION — อ้างอิง BR-PREP-006, BR-PREP-007
...
| BR-GPS-009 | เกณฑ์คุณภาพ GPS (ความแม่นยำ, การลอง Check-in ซ้ำ) ยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
| BR-RECIPIENT-007 | ฟิลด์ข้อมูลผู้รับสินค้าที่มีสิทธิ์ใช้ Correction Action ได้อย่างชัดเจนยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
* **VR-RECIPIENT-003a** — ก่อนดำเนินการ Correction Action ต่อข้อมูลผู้รับสินค้า ตรวจสอบว่าผู้กระทำการคือ Admin หรือ Super Admin และมีเหตุผลบังคับ — Timing: BEFORE_CORRECTION — อ้างอิง BR-RECIPIENT-005, BR-RECIPIENT-006 — Enforcement: CORRECTION_REQUIRED
...
| BR-QTY-012 | ระบบไม่กำหนดข้อกำหนด Barcode, Serial Number, Lot Number บังคับสากล หรือส่วนต่างที่อนุญาตให้ส่งเกิน (Over-delivery Tolerance) | ADVISORY | เปิด — ดู §40 |
...
| BR-ATTEMPT-011 | จำนวน Delivery Attempt สูงสุดยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
| BR-EXTERNAL-009 | ข้อกำหนดขั้นต่ำที่แน่นอนของหลักฐานผู้ส่งสินค้าภายนอกยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
Returned-Goods Status เป็นสถานะที่แยกจาก Main Task Status อย่างสมบูรณ์ มี 3 ค่า: NOT_REQUIRED, PENDING_RETURN, RETURN_CONFIRMED
...
| BR-RETURN-008 | ประเด็นว่าการยกเลิกงานสร้างรายการติดตามการคืนสินค้าภาคบังคับโดยอัตโนมัติหรือไม่ ยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
| BR-REOPEN-010 | จำนวนรอบ Reopen สูงสุดที่อนุญาตยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
| BR-OVERRIDE-010 | ผลที่ตามมาเมื่อ Super Admin ปฏิเสธ (Reject) Override และกำหนดเวลาการทบทวนยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
| BR-DATA-003 | การเปลี่ยนแปลงข้อมูลหลักลูกค้าหรือปลายทางต้องไม่เขียนทับ Historical Destination Snapshot ที่ใช้จริง | HARD_BLOCK | Topic 5 §8 |
...
| BR-SECURITY-009 | การเปิดเผยข้อมูลอ่อนไหวแบบไม่ปิดบังระหว่างการสืบสวนอย่างเป็นทางการยังไม่ได้ข้อสรุป | ADVISORY | เปิด — ดู §40 |
...
| 24 | Stock แก้ไขข้อมูลการเตรียมสินค้าหลัง IN_TRANSIT โดยตรง | BR-PREP-005, BR-PREP-006 | รายงานให้ Admin/Super Admin สร้าง Correction/Exception Record |
...
| 7 | BR-TASK-003 | Task Creation | ข้อมูลปลายทางต้องเพียงพอต่อการปฏิบัติงาน | ชื่อ+ที่อยู่ปลายทางครบ | BEFORE_PREPARATION | HARD_BLOCK | Dispatcher/Admin | Task-level | เข้าสู่ WAITING_PREPARATION | คง DRAFT ไว้ | แก้ไขข้อมูลก่อน | บังคับ | Topic 2 §5.2 | — |
...
| 12 | BR-TASK-009 | Task Creation | ตรึง Historical Destination Snapshot เมื่อเริ่มจัดส่ง | Snapshot ถูกสร้างแยกจากข้อมูลหลัก | BEFORE_DELIVERY_START | HARD_BLOCK | ระบบ | Attempt-level | เดินทางตามข้อมูลที่ตรึงไว้ | ข้อมูลย้อนหลังไม่ถูกต้อง | ไม่มี | บังคับ | Topic 5 §8 | Immutable |
...
| 23 | BR-PREP-005 | Preparation | Stock Edit Lock เริ่มที่ IN_TRANSIT | Task ยังไม่เข้า IN_TRANSIT | BEFORE_DELIVERY_START | HARD_BLOCK | Stock | Attempt-level | แก้ไขข้อมูลเตรียมสินค้าได้ | ล็อกการแก้ไข | Correction/Exception Record โดย Admin/Super Admin | บังคับ | Topic 3 §14.2 | — |
| 24 | BR-PREP-007 | Preparation | แก้ไขข้อมูลเตรียมสินค้าหลัง IN_TRANSIT ต้องผ่าน Admin/Super Admin | มีบันทึก Correction/Exception | BEFORE_CORRECTION | CORRECTION_REQUIRED | Admin/Super Admin | Attempt-level | แก้ไขที่มีการควบคุม | ปฏิเสธการแก้ไขโดย Stock | ไม่มี | บังคับ | Topic 4 §27 | — |
...
| 113 | BR-DATA-003 | Data Governance | การเปลี่ยนข้อมูลหลักลูกค้าไม่เขียนทับ Snapshot | Snapshot คงเดิมแม้ข้อมูลหลักเปลี่ยน | ON_HISTORY_APPEND | HARD_BLOCK | ระบบ | Attempt-level | Snapshot ถูกต้อง | Snapshot ถูกเขียนทับ | ไม่มี | บังคับ | Topic 5 §8 | — |
...
* **Evidence**: BR-EXTERNAL-005 — หลักฐานชุดเดียวกับงานภายใน
...
* **Preconditions**: Task อยู่ในสถานะ Terminal (COMPLETED หรือ CANCELLED)
* **Actor**: Admin
...
* **ผลเมื่อไม่ผ่าน**: บล็อกการ Reopen (BLOCK_NORMAL_ACTION)
* **เส้นทางทางเลือก**: Correction Action หากไม่จำเป็นต้อง Reopen ทั้ง Task
* **Timeline**: บังคับ | **Audit Log**: บังคับ
* **ประวัติที่ต้องรักษาไว้**: การปิดงานหรือยกเลิกเดิม, Attempt เดิมทั้งหมด

### Checklist 19: Reopen by Super Admin

* **Preconditions**: Task อยู่ในสถานะ Terminal หรือกรณีข้อยกเว้นที่เกินอำนาจ Admin
* **Actor**: Super Admin
...
* **Preconditions**: Task ปิดแล้ว (COMPLETED) และพบข้อมูลผู้รับสินค้าผิดพลาด
* **Actor**: Admin หรือ Super Admin
* **Permission**: BR-CORRECTION-001, BR-RECIPIENT-005
* **Status**: ไม่เปลี่ยนแปลง (BR-CORRECTION-002)
* **Data**: ค่าดั้งเดิม ค่าใหม่ เหตุผล (บังคับ)
* **Evidence**: หลักฐานสนับสนุนการแก้ไขเมื่อเกี่ยวข้อง (CONDITIONAL)
* **Quantity**: ไม่เกี่ยวข้องโดยตรง
...
* **Permission**: BR-PREP-006, BR-PREP-007
* **Status**: ไม่เปลี่ยนแปลง
* **Data**: รายงานข้อผิดพลาดจาก Stock, บันทึกแก้ไขหรือข้อยกเว้นโดย Admin/Super Admin
* **Evidence**: ประวัติการเตรียมสินค้าเดิมต้องยังคงถูกรักษาไว้ครบถ้วน
* **Quantity**: ความแตกต่างระหว่างจำนวนที่บันทึกไว้กับจำนวนจริง
* **ผลเมื่อผ่าน**: บันทึกแก้ไขหรือข้อยกเว้นสำเร็จ (ROUTE_TO_CORRECTION)
...
* **Timeline**: ไม่บังคับโดยตรง | **Audit Log**: ควรบันทึกเมื่อระบบรองรับ (ประเด็นเปิด)
...
* เส้นทางทางเลือก: รายงานให้ Admin/Super Admin สร้าง Correction/Exception Record | Main Task Status ที่ได้: คง IN_TRANSIT
* สถานะประกอบ: ไม่เปลี่ยนแปลง | Timeline: บังคับ (บันทึกความพยายามที่ถูกปฏิเสธ) | Audit Log: บังคับ
...
* สถานะประกอบ: ไม่เปลี่ยนแปลง | Timeline: ไม่เกี่ยวข้องโดยตรง | Audit Log: ควรบันทึกความพยายามที่ถูกปฏิเสธ (ประเด็นเปิดว่าบังคับหรือไม่)
...
* **กฎที่ไม่มีแหล่งที่มาที่อนุมัติแล้วต้องถูกทำเครื่องหมายเป็นข้อเสนอหรือ Open Business Decision** ไม่ใช่ถูกนำเสนอราวกับเป็นกฎที่อนุมัติแล้ว — ดูหัวข้อ [40](#40-open-business-decisions)
* **ข้อกำหนดการ Implement ในอนาคตควรตรวจสอบย้อนกลับไปยัง Business Rule ID หนึ่งรายการหรือมากกว่าในเอกสารฉบับนี้เสมอ** เพื่อรักษาความสอดคล้องกับ Topics 1–5 ตลอดวงจรการพัฒนา

## 40. Open Business Decisions

หัวข้อนี้รวบรวมเฉพาะประเด็นทางธุรกิจที่**ยังไม่ได้รับการยืนยัน**จากเจ้าของธุรกิจ ซึ่งรวบรวมมาจากประเด็นเปิดที่ยังคงค้างอยู่ใน [[02 - Workflow การทำงานของระบบ Dispatch]] หัวข้อ 20, [[03 - บทบาทและสิทธิ์ผู้ใช้งาน]] หัวข้อ 27, [[04 - สถานะของงานและกติกาการเปลี่ยนสถานะ]] หัวข้อ 32 และ [[05 - ข้อมูล หลักฐาน และรายละเอียดที่ต้องจัดเก็บในแต่ละงาน]] หัวข้อ 39 เอกสารฉบับนี้จะไม่ตัดสินใจแทนในประเด็นเหล่านี้ ประเด็นที่ Topics 1–5 ได้อนุมัติไว้แล้วอย่างชัดเจน**ไม่ปรากฏ**ในรายการนี้ เนื่องจากถูกนำไปใช้เป็น Business Rule ที่อนุมัติแล้วในหัวข้อต่าง ๆ ข้างต้นโดยตรง

### A. Task Creation และ Reference Data

1. เลขที่เอกสารอ้างอิงทางธุรกิจรายการใดบ้างที่เป็นข้อบังคับ
2. Customer Master Selection เป็นข้อบังคับหรือไม่
3. อนุญาตให้กรอกปลายทางแบบข้อความอิสระ (Free-text Destination) หรือไม่
4. ฟิลด์ปลายทางใดบ้างต้องถูกตรึงเป็น Historical Snapshot
5. ลำดับความสำคัญ (Priority) เป็นข้อบังคับหรือไม่
6. ประเภทงานใดบ้างนอกเหนือจากงานจัดส่งสินค้าจะกลายเป็นความสามารถที่ Active ใน Phase 1

### B. Preparation และ Quantity

7. ผู้มีอำนาจแก้ไขข้อมูลการเตรียมสินค้าหลัง IN_TRANSIT (Admin เองหรือจำกัดเฉพาะ Super Admin)
8. ข้อผิดพลาดการเตรียมสินค้ากรณีใดต้องใช้ Reopen เทียบกับ Correction/Exception Record เพียงอย่างเดียว
9. จำเป็นต้องมีการรับทราบของ Stock เพิ่มเติมสำหรับสินค้าที่ถูกส่งคืนหรือไม่
10. อนุญาตให้ส่งเกินจำนวนที่วางแผน (Over-delivery) หรือไม่
11. มีส่วนต่างที่ยอมรับได้ของจำนวน (Quantity Tolerance) หรือไม่
12. จำเป็นต้องบันทึก Serial Number หรือไม่
13. จำเป็นต้องบันทึกหมายเลข Lot หรือ Batch หรือไม่
14. การสแกน Barcode เป็นขอบเขตอนาคตหรือไม่

### C. Assignment และบุคลากร

15. พนักงานที่ร่วมปฏิบัติงาน (Supporting/Co-traveling Employee) ควรถูกบันทึกในระบบอย่างไร
16. พนักงานที่ร่วมปฏิบัติงานสามารถให้ข้อมูลหรือหลักฐานได้หรือไม่
17. พนักงานทดแทนชั่วคราวควรได้รับสิทธิ์เข้าถึง Task อย่างไร
18. หนึ่งพนักงานสามารถถือ Task ที่ Active พร้อมกันได้มากกว่าหนึ่งรายการหรือไม่
19. การเปลี่ยนตัวผู้รับผิดชอบ (Reassignment) ทุกกรณีต้องมีเหตุผลหรือไม่

### D. GPS และปลายทาง

20. เกณฑ์ความแม่นยำของ GPS ที่ยอมรับได้
21. ข้อมูลความน่าเชื่อถือของ GPS (Reliability) เป็นข้อบังคับหรือไม่
22. อนุญาตให้ทำ Check-in ซ้ำได้หรือไม่
23. GPS ล้มเหลวต้องมีคำอธิบายเพิ่มเติมหรือไม่
24. มีการยืนยันปลายทางด้วยตนเอง (Manual Confirmation) หรือไม่
25. การเข้าถึงพิกัด GPS ควรสร้างเหตุการณ์ Audit Log เฉพาะทางหรือไม่

### E. รูปภาพและลายเซ็น

26. วิธีการเก็บลายเซ็นทางเทคนิคที่แน่นอน
27. ภาพถ่ายเอกสารที่ลงชื่อแล้วใช้แทนลายเซ็นบนหน้าจอได้หรือไม่
28. เนื้อหาที่บังคับต้องปรากฏในรูปหลักฐานการส่งมอบ
29. เนื้อหาที่บังคับต้องปรากฏในรูปหลังโหลดสินค้า
30. ความละเอียดขั้นต่ำของภาพ
31. ขนาดไฟล์สูงสุดที่อนุญาต
32. รูปแบบไฟล์ที่ยอมรับ
33. เกณฑ์การปฏิเสธหลักฐานที่มีคุณภาพต่ำ
34. การดูลายเซ็นลูกค้าควรสร้างเหตุการณ์ Audit Log เฉพาะทางหรือไม่

### F. Delivery Attempt และการปิดงาน

35. จำนวน Delivery Attempt สูงสุด
36. WAITING_NEXT_ATTEMPT จำเป็นต้องมีวันนัดครั้งถัดไปเป็นข้อมูลบังคับหรือไม่
37. มีจำนวนการเลื่อนนัดสูงสุดหรือไม่
38. COMPLETED เกิดขึ้นได้หรือไม่ในขณะที่ยังมีภาระหน้าที่คืนสินค้าที่แยกต่างหากค้างอยู่
39. Emergency Override สามารถนำไปสู่ WAITING_NEXT_ATTEMPT แทนสถานะสุดท้ายได้หรือไม่
40. การกระทำใดของ Admin ที่ต้องบังคับให้ระบุเหตุผลเพิ่มเติมนอกเหนือจากที่อนุมัติแล้ว

### G. ผู้ส่งสินค้าภายนอก

41. ข้อกำหนดขั้นต่ำของหลักฐานที่ผู้ส่งสินค้าภายนอกต้องให้ได้
42. หมายเลขติดตามพัสดุของผู้ส่งภายนอกเป็นข้อบังคับหรือไม่
43. ชื่อบริษัทผู้ให้บริการภายนอกเป็นข้อบังคับหรือไม่
44. บทบาทใดสามารถดูค่าจัดส่งของผู้ให้บริการภายนอกในรายงานของฝ่ายบริหารได้
45. หลักฐานของผู้ส่งภายนอกจำเป็นต้องมีการตรวจสอบแยกต่างหากโดย Admin หรือไม่
46. ค่าจัดส่งภายนอกจำเป็นต้องมีการอนุมัติหรือไม่

### H. การยกเลิกและการคืนสินค้า

47. การยกเลิกงานหลังสินค้าออกจากบริษัทควรสร้างรายการติดตามการคืนสินค้าภาคบังคับโดยอัตโนมัติหรือไม่
48. ข้อกำหนดหลักฐานที่บังคับสำหรับการคืนสินค้า
49. Task ที่ถูกยกเลิกพร้อม Returned-Goods = PENDING_RETURN สามารถถูก Reopen ได้หรือไม่
50. การยืนยันคืนสินค้าต้องตรงกับจำนวนที่คาดไว้เป๊ะหรือไม่
51. วิธีจัดการสินค้าคืนที่ขาดหายหรือเสียหาย
52. จำเป็นต้องมีการทบทวนของ Super Admin สำหรับข้อพิพาทการคืนสินค้าที่ยังไม่คลี่คลายหรือไม่

### I. Reopen และ Correction

53. อนุญาตให้มีรอบ Reopen มากกว่าหนึ่งรอบหรือไม่
54. จำนวนรอบ Reopen สูงสุด
55. ฟิลด์ข้อมูลผู้รับสินค้าที่มีสิทธิ์ใช้ Correction Action ได้อย่างชัดเจน
56. Correction Action ต้องแจ้งเตือน Management/Auditor หรือไม่
57. Admin มีอำนาจแก้ไขข้อมูลการเตรียมสินค้าได้เองหรือไม่
58. การทำให้หลักฐานเป็นโมฆะต้องมีการอนุมัติแยกต่างหากหรือไม่
59. ใครมีอำนาจอนุมัติการทำให้หลักฐานเป็นโมฆะ
60. สามารถเพิ่มเอกสารประกอบภายหลังปิดงานได้โดยไม่ต้องผ่าน Correction Action หรือไม่

### J. Emergency Override

61. Super Admin Review จำเป็นต้องมีกำหนดเวลา (Review Deadline) หรือไม่
62. ผลที่ตามมาเมื่อ Super Admin ปฏิเสธการใช้ Emergency Override
63. Super Admin สามารถย้อนกลับ (Reverse) ผลการจัดส่งสุดท้ายได้หรือไม่
64. การใช้ Emergency Override ซ้ำหลายครั้งควรกระตุ้นการแจ้งเตือนความเสี่ยงอัตโนมัติหรือไม่
65. หนึ่งบุคคลสามารถทั้งริเริ่มและทบทวน Override เดียวกันได้หรือไม่ (นอกเหนือจากที่กำหนดไว้แล้วว่า Admin ริเริ่มและ Super Admin ทบทวนเสมอ)
66. เหตุผลของ Override จำเป็นต้องใช้หมวดหมู่ที่ควบคุมไว้ล่วงหน้าหรือไม่
67. การใช้ Override ซ้ำหลายครั้งควรระงับอำนาจของผู้กระทำการหรือไม่
68. Override สามารถปิดงานที่มีจำนวนสินค้าที่ไม่ทราบค่าโดยสิ้นเชิงได้หรือไม่

### K. ข้อมูลอ่อนไหวและการส่งออก

69. Dispatcher สามารถส่งออกรายงานประเภทใดได้บ้าง
70. Management/Auditor สามารถส่งออกรายงานประเภทใดได้บ้าง
71. กระบวนการเปิดเผยข้อมูลแบบไม่ปิดบังระหว่างการสืบสวนอย่างเป็นทางการ
72. สิทธิ์การเข้าถึงจำเป็นต้องถูกจำกัดตามแผนก สาขา ทีม หรือพื้นที่ปฏิบัติงานหรือไม่
73. ระยะเวลาการปิดบังเบอร์โทรผู้รับสินค้า
74. ระยะเวลาการเก็บรักษาลายเซ็น
75. ระยะเวลาการเก็บรักษาพิกัด GPS
76. ระยะเวลาการเก็บรักษาหลักฐานโดยทั่วไป
77. การส่งออกหลักฐานจำเป็นต้องระบุเหตุผลหรือไม่
78. การส่งออกหลักฐานจำเป็นต้องมีลายน้ำ (Watermark) หรือไม่
79. การเข้าถึงข้อมูลอ่อนไหวจำเป็นต้องมีการทบทวนสิทธิ์เป็นระยะหรือไม่
80. ลูกค้าจะได้รับสิทธิ์เข้าถึงหลักฐานโดยตรงในอนาคตหรือไม่

> [!note]
> รายการข้างต้นคือประเด็นที่**ยังไม่ได้รับการยืนยัน**เท่านั้น หากมีการอนุมัติในภายหลัง รายการที่เกี่ยวข้องจะถูกนำออกจากหัวข้อนี้และย้ายไปเป็น Business Rule ที่อนุมัติแล้วในหัวข้อที่เกี่ยวข้อง ตามรูปแบบเดียวกับที่ Topics 2–5 ใช้เมื่อประเด็นเปิดของตนได้รับการอนุมัติในภายหลัง
...
* เอกสารฉบับนี้ไม่ได้กำหนดกฎใหม่ด้วยตนเอง ทุกกฎตรวจสอบย้อนกลับไปยัง Topics 1–5 ได้ และประเด็นที่ยังไม่ได้รับการยืนยันถูกรวบรวมไว้ในหัวข้อ [40](#40-open-business-decisions) เพื่อรอการตัดสินใจจากเจ้าของธุรกิจก่อนนำไปใช้ในการออกแบบ Validation Engine, Test Case และ Implementation Specification ในระยะถัดไป
