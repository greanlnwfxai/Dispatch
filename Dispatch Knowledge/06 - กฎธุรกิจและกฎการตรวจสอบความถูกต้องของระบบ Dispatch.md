...
| BR-RETURN-006 | ก่อน Admin ยืนยัน RETURN_CONFIRMED ต้องมี Core Return Record ที่ตรวจสอบย้อนกลับได้ ได้แก่ source Task, source Delivery Attempt, expected returned quantity, actual returned quantity, returned-item condition, shortage/damage details เมื่อมี, confirming Admin, confirmation timestamp, Timeline และ Audit Log; additional evidence เป็นข้อบังคับเมื่อมี Risk Trigger ตาม BDR-RETURN-002 Option C | HARD_BLOCK | Topic 5 §22; อนุมัติ 2026-07-21 — BDR-RETURN-002 Option C |
...
* **VR-RETURN-001a** — ก่อนยืนยันรับคืนสินค้า ตรวจสอบว่าผู้กระทำการคือ Admin, Core Return Record ครบตาม BDR-RETURN-002 Option C, และเมื่อมี Risk Trigger ได้แก่ quantity mismatch, damage, opened packaging, incomplete external-courier information, dispute หรือ Formal Investigation ต้องมี additional evidence ที่จำเป็นและสอดคล้องกับ Data Minimization — Timing: BEFORE_RETURN_CONFIRMATION — อ้างอิง BR-RETURN-003, BR-RETURN-006, BR-SECURITY-008 — Enforcement: HARD_BLOCK
...
| BR-OVERRIDE-010 | เมื่อ Super Admin ปฏิเสธ (Reject) การใช้ Emergency Override ต้องบันทึกเป็น Review Finding และเลือก Controlled Action แยกตามข้อเท็จจริงและความรุนแรง; Reject ไม่ลบหรือเขียนทับ Override เดิม ไม่เปลี่ยน Main Task Status อัตโนมัติ และไม่ Reopen/Cancel อัตโนมัติ | REVIEW_REQUIRED | อนุมัติ 2026-07-21 — BDR-OVERRIDE-003 Option C; กำหนดเวลาทบทวนยังเปิดภายใต้ BDR-OVERRIDE-002 |
...
| BR-REVIEW-004 | ผลการทบทวนที่เป็นไปได้ ได้แก่ ยอมรับ, ยอมรับพร้อมหมายเหตุ, ร้องขอข้อมูลเพิ่มเติม, ต้องแก้ไข, เปิดงานกลับมาแก้ไข, ต้องแก้ไขหลักฐาน, ต้องแก้ไขผลการจัดส่ง, ยกระดับเพื่อสืบสวน หรือ Reject ในฐานะ Review Finding ที่ต้องเลือก Controlled Action แยกตาม BDR-OVERRIDE-003 Option C | REVIEW_REQUIRED | Topic 3 §16.5; อนุมัติ 2026-07-21 — BDR-OVERRIDE-003 Option C |
| BR-REVIEW-005 | REVIEW_ACCEPTED ไม่ได้หมายความว่าหลักฐานที่ขาดอยู่เดิมมีอยู่จริง | ADVISORY | Topic 4 §25 |
| BR-REVIEW-006 | เฉพาะ Super Admin เท่านั้นที่ทำการทบทวนอย่างเป็นทางการได้ — Dispatcher, Stock, พนักงานส่งสินค้า และ Management/Auditor ไม่มีสิทธิ์อนุมัติหรือทบทวน; ผู้ริเริ่มและผู้ทบทวน Override เดียวกันต้องต่างบุคคลตามปกติ แต่ same-person review อนุญาตเฉพาะ Emergency Continuity เมื่อไม่มี Super Admin คนอื่นและการรอจะกระทบปฏิบัติการอย่างมีนัยสำคัญ พร้อมบันทึก compensating controls ครบถ้วน | HARD_BLOCK / EXCEPTION_ONLY | Topic 3 §16.5; อนุมัติ 2026-07-21 — BDR-OVERRIDE-006 Option B |
...
* **VR-REVIEW-001a** — ก่อนเริ่มทบทวน ตรวจสอบว่าผู้กระทำการคือ Super Admin และโดยปกติต้องไม่ใช่บุคคลเดียวกับ initiator ของ Override; same-person review อนุญาตเฉพาะ Emergency Continuity พร้อม reason, unavailable reviewer explanation, actor identity, timestamp, Same-person Review marker, Timeline และ Audit Log — Timing: BEFORE_OVERRIDE_REVIEW — อ้างอิง BR-REVIEW-006 — Enforcement: HARD_BLOCK / EXCEPTION_ONLY
* **VR-REVIEW-002a** — เมื่อบันทึกผลการทบทวน ตรวจสอบว่าบันทึก Override ต้นฉบับยังคงอยู่ครบถ้วนและไม่ถูกเขียนทับ; หากผลเป็น Reject ต้องบันทึกเป็น Review Finding พร้อม reason/actor/timestamp/Timeline/Audit Log และ follow-up เป็น Controlled Action แยกโดยไม่เปลี่ยน Main Task Status อัตโนมัติ — Timing: BEFORE_OVERRIDE_REVIEW — อ้างอิง BR-REVIEW-003, BR-REVIEW-004, BR-OVERRIDE-010 — Enforcement: HARD_BLOCK / REVIEW_REQUIRED
...
| BR-SECURITY-009 | การเปิดเผยข้อมูลอ่อนไหวแบบไม่ปิดบังอนุญาตเฉพาะ Formal Investigation ที่มี Case Reference และต้องผ่าน case-by-case Security/Privacy Review ภายใต้นโยบายที่ Product Owner / User อนุมัติ; ต้องบันทึก reason, necessity, exact data scope, authorized recipients, access duration, export restriction, actor, timestamp, Case Reference และ Audit Log; ห้าม indefinite access และห้าม access นอก Case scope | HARD_BLOCK | อนุมัติ 2026-07-21 — BDR-PRIVACY-001 Option B |

### Validation Rules

* **VR-SECURITY-001a** — ก่อนแสดงเบอร์โทรผู้รับสินค้าหรือลายเซ็นลูกค้า ตรวจสอบบทบาทผู้ขอเข้าถึง, ความสัมพันธ์กับ Task, สถานะการปิดงาน และหากเป็น unmasked Formal Investigation ต้องมี Case Reference, approved data scope, authorized recipients, access duration และ Audit Log ตาม BR-SECURITY-009 — Timing: BEFORE_SENSITIVE_DATA_ACCESS — อ้างอิง BR-SECURITY-001, BR-SECURITY-004, BR-SECURITY-009 — Enforcement: HARD_BLOCK
* **VR-SECURITY-002a** — ก่อนส่งออกรายงานหรือหลักฐาน ตรวจสอบสิทธิ์การส่งออกของบทบาท ระดับความอ่อนไหวของข้อมูลที่ส่งออก และ export restriction ของ Case เมื่อเป็น Formal Investigation — Timing: BEFORE_EXPORT — อ้างอิง BR-SECURITY-007, BR-SECURITY-009 — Enforcement: HARD_BLOCK
...
| 83 | BR-RETURN-003 | Returned-Goods | Admin ยืนยันรับคืนสินค้าอย่างเป็นทางการ | ผู้กระทำการ = Admin และ Core Return Record ครบ | BEFORE_RETURN_CONFIRMATION | HARD_BLOCK | Admin | Task-level | Returned-Goods = RETURN_CONFIRMED | DENY_UNAUTHORIZED_ACCESS หรือคง PENDING_RETURN จนข้อมูลครบ | ไม่มี | บังคับ | Topic 3 §19; BDR-RETURN-002 Option C | Admin เท่านั้น |
...
| 104 | BR-REVIEW-004 | Override Review | Reject เป็น Review Finding พร้อม Controlled Action แยกเมื่อเกี่ยวข้อง | Review Outcome หรือ Review Finding มี reason/actor/timestamp | BEFORE_OVERRIDE_REVIEW | REVIEW_REQUIRED | Super Admin | Task-level | บันทึกผลทบทวนหรือ Reject Finding สำเร็จ | ผลทบทวนไม่ชัดเจน | Controlled Action แยก เช่น correction, Reopen หรือ ESCALATED เมื่อจำเป็น | บังคับ | Topic 3 §16.5; BDR-OVERRIDE-003 Option C | Main Task Status ไม่เปลี่ยนอัตโนมัติ |
| 105 | BR-REVIEW-006 | Override Review | เฉพาะ Super Admin ทำการทบทวนอย่างเป็นทางการ และ reviewer ต้องต่างจาก initiator ตามปกติ | ผู้กระทำการ = Super Admin; reviewer ≠ initiator เว้นแต่ Emergency Continuity | BEFORE_OVERRIDE_REVIEW | HARD_BLOCK / EXCEPTION_ONLY | Super Admin | Task-level | ทบทวนสำเร็จพร้อม marker เมื่อ same-person | DENY_UNAUTHORIZED_ACCESS หรือ require Emergency Continuity controls | ไม่มี | บังคับ | Topic 3 §16.5; BDR-OVERRIDE-006 Option B | ไม่เพิ่ม Role ใหม่ |
...
| 127 | VR-OPEN-001 | Open Decision Boundary | ประเด็นที่ยังไม่อนุมัติต้องไม่ถูก Implement เหมือนอนุมัติแล้ว | รายการตรงกับ BDR Decision ID ที่ยังไม่อนุมัติในทะเบียนหรือไม่ | (ทุกช่วง) | ADVISORY | Dispatcher/Admin/Super Admin | Task-level | ดำเนินการตามค่า Default หรือผลอนุมัติล่าสุดใน Topic 7 เท่านั้น | ยกระดับเป็นคำถามให้เจ้าของธุรกิจ | รอการอนุมัติ | ไม่บังคับ | Topic 2 §20; Topic 3 §27; Topic 4 §32; Topic 5 §39; [[07 - ขอบเขต MVP และทะเบียนการตัดสินใจทางธุรกิจ]] | BDR-RETURN-002, BDR-OVERRIDE-003, BDR-OVERRIDE-006, BDR-PRIVACY-001 อนุมัติแล้ว 2026-07-21; BDR-RETURN-007 และ BDR-RETURN-009 ยังเปิด |
