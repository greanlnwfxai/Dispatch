...
* เว้นช่องการตัดสินใจสุดท้ายว่างไว้สำหรับ User ทุกประเด็น
...
เอกสารฉบับนี้เป็นเอกสารระดับ**การวิเคราะห์การตัดสินใจ** (Decision Analysis) เท่านั้น **ไม่ใช่**บันทึกการอนุมัติ (Approval Record) และ**ไม่ใช่**เอกสารออกแบบทางเทคนิค
...
* แบบฟอร์มการตัดสินใจของ User ที่ว่างเปล่า (Blank User Decision Record) สำหรับทุกประเด็น P0
* ตารางสรุปสำหรับ User (User Decision Worksheet) ที่มีสถานะ "รอการตัดสินใจ" เสมอ
* แผนงานถัดไปหลัง User อนุมัติ (Post-Decision Knowledge Update Plan / DISPATCH-BDR-P0-002) — **ไม่ดำเนินการในภารกิจนี้**
...
* คำตอบสุดท้ายของประเด็น P0 ใด ๆ — ทุกช่องการตัดสินใจเว้นว่างไว้เสมอ
...
3. **Topic 8 ไม่เปลี่ยน Decision Status ของรายการใดใน Topic 7** ไม่ว่ากรณีใด — Decision Status ยังคงเป็น MUST_DECIDE_BEFORE_MVP จนกว่า Topic 7 จะถูกปรับปรุงอย่างเป็นทางการผ่านภารกิจแยกต่างหาก (DISPATCH-BDR-P0-002)
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | BDR-CUSTOMER-001 | Customer Master Selection เป็นข้อบังคับหรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | PRODUCT_SCOPE, CUSTOMER_DESTINATION, DATA_REQUIREMENT | Product Owner / User | ก่อนเริ่มออกแบบ MVP-02 | MVP-02, MVP-04 | BR-TASK-003 | Topic 5 §6, §8; Topic 7 §15 กลุ่ม A แถว 2, §16 ข้อ 2 |
| 2 | BDR-CUSTOMER-002 | อนุญาตให้กรอกปลายทางแบบข้อความอิสระ (Free-text) หรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | PRODUCT_SCOPE, CUSTOMER_DESTINATION | Product Owner / User | ก่อนเริ่มออกแบบ MVP-02 | MVP-02, MVP-04 | BR-TASK-003 | Topic 5 §6, §8; Topic 7 §15 กลุ่ม A แถว 3, §16 ข้อ 2 |
| 3 | BDR-EVIDENCE-001 | วิธีการทางเทคนิคที่แน่นอนของการเก็บลายเซ็น (วาดบนหน้าจอ, ถ่ายภาพเอกสาร หรือวิธีอื่น) | MUST_DECIDE_BEFORE_MVP | P0 | EVIDENCE, TECHNICAL_ARCHITECTURE, UX | Product Owner / User (ร่วม UX และ Technical Architecture) | ก่อนเริ่มออกแบบ MVP-07 | MVP-07, MVP-09, MVP-10 | BR-EVIDENCE-*, BR-RECIPIENT-003 | Topic 2 §5.13; Topic 5 §17, §39 กลุ่ม B; Topic 7 §15 กลุ่ม E แถว 26, §16 ข้อ 4 |
| 4 | BDR-EVIDENCE-002 | ภาพถ่ายเอกสารที่ลงชื่อแล้วสามารถใช้แทนลายเซ็นบนหน้าจอได้หรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | EVIDENCE, WORKFLOW | Product Owner / User | ก่อนเริ่มออกแบบ MVP-07 | MVP-07, MVP-09 | BR-RECIPIENT-003 | Topic 2 §5.13; Topic 5 §39 กลุ่ม B; Topic 7 §15 กลุ่ม E แถว 27, §16 ข้อ 5 |
| 5 | BDR-EXTERNAL-001 | ข้อกำหนดขั้นต่ำของหลักฐานที่ผู้ส่งสินค้าภายนอกต้องให้ได้ | MUST_DECIDE_BEFORE_MVP | P0 | EXTERNAL_COURIER, EVIDENCE, WORKFLOW | Product Owner / User (ร่วม Admin Operations) | ก่อนเริ่มออกแบบ MVP-10 | MVP-10 | BR-EXTERNAL-005, BR-EXTERNAL-009 | Topic 5 §21, §39 กลุ่ม F; Topic 7 §15 กลุ่ม G แถว 41, §16 ข้อ 7–8 |
| 6 | BDR-PREP-001 | ผู้มีอำนาจแก้ไขข้อมูลการเตรียมสินค้าหลัง IN_TRANSIT (Admin เองได้ หรือจำกัดเฉพาะ Super Admin) | MUST_DECIDE_BEFORE_MVP | P0 | PERMISSION, WORKFLOW, DATA_REQUIREMENT | Product Owner / User (ร่วม Super Admin Governance) | ก่อนเริ่มพัฒนา MVP-03 | MVP-03, MVP-16 | BR-PREP-007, BR-PREP-008 | Topic 3 §14.2; Topic 4 §27; Topic 7 §15 กลุ่ม B แถว 7, §16 ข้อ 9 |
| 7 | BDR-PREP-004 | Admin สามารถสร้างบันทึกแก้ไขสำหรับข้อมูลการเตรียมสินค้าหลังเริ่มจัดส่งได้เอง หรือจำกัดเฉพาะ Super Admin เท่านั้น | MUST_DECIDE_BEFORE_MVP | P0 | PERMISSION, CORRECTION_ACTION | Product Owner / User (ร่วม Super Admin Governance) | ก่อนเริ่มพัฒนา MVP-03/MVP-16 | MVP-03, MVP-16 | BR-PREP-007, BR-PREP-008 | Topic 3 §27, §14.2; Topic 4 §32; Topic 5 §39 กลุ่ม G; Topic 7 §15 กลุ่ม J แถว 57 |
| 8 | BDR-CORRECTION-001 | ฟิลด์ข้อมูลผู้รับสินค้ารายการใดบ้างที่มีสิทธิ์ใช้ Correction Action ได้อย่างชัดเจน | MUST_DECIDE_BEFORE_MVP | P0 | CORRECTION_ACTION, RECIPIENT, GOVERNANCE | Product Owner / User (ร่วม Admin Operations) | ก่อนเริ่มพัฒนา MVP-16 | MVP-16 | BR-RECIPIENT-007 | Topic 3 §27, §16 (recipient fields); Topic 5 §39 กลุ่ม A; Topic 7 §15 กลุ่ม I แถว 55, §16 ข้อ 12 |
| 9 | BDR-RETURN-003 | Task ที่ถูกยกเลิกพร้อม Returned-Goods = PENDING_RETURN สามารถถูก Reopen ได้หรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | REOPEN, CANCELLATION, RETURNED_GOODS, STATUS_TRANSITION | Product Owner / User (ร่วม Super Admin Governance) | ก่อนเริ่มพัฒนา MVP-13 | MVP-12, MVP-13 | BR-REOPEN-005, BR-RETURN-002 | Topic 4 §32 ข้อ 22; Topic 7 §15 กลุ่ม H แถว 49, §16 ข้อ 25 |

> [!warning]
> ตารางข้างต้นมีทั้งหมด **9 แถว** — ตรงตามจำนวน P0 ที่ยืนยันแล้วในหัวข้อ 4.2 **ไม่มีรายการ P1, P2 หรือ P3 ปรากฏในตารางนี้หรือในการวิเคราะห์เจาะลึกของเอกสารฉบับนี้** Decision Status ทุกแถวคัดลอกมาจาก Topic 7 โดยตรง **ไม่มีรายการใดถูกทำเครื่องหมายเป็น APPROVED**
...
* **B. Current Classification** — Decision ID, Decision Status, Priority, เจ้าของ, Approval Checkpoint, Impact Categories, MVP Feature Group, Rule ID ที่เกี่ยวข้อง, Source Topics
...
* **I. User Decision Record** — แบบฟอร์มว่างเปล่าที่มีสถานะ `รอการตัดสินใจจาก User` เสมอ ไม่กรอกทางเลือกล่วงหน้า
...
* **Decision ID**: BDR-CUSTOMER-001
* **Selected option**: _______________ (`รอการตัดสินใจจาก User`)
* **Approved wording**: _______________
* **Conditions or limitations**: _______________
* **Decision owner**: _______________
* **Approved by**: _______________
* **Approval date**: _______________
* **Effective MVP Feature Groups**: _______________
* **Topics requiring update**: _______________
* **Related Rule IDs requiring review**: _______________
* **Notes**: _______________

> รอการตัดสินใจจาก User
...
* **เงื่อนไขที่ต้องมีก่อนใช้ข้อเสนอนี้ได้**: ต้องได้รับการอนุมัติสอดคล้องกับ BDR-CUSTOMER-001 Option C ในคราวเดียวกัน — **ห้ามอนุมัติ BDR-CUSTOMER-002 Option A/C พร้อมกับ BDR-CUSTOMER-001 Option A** เนื่องจากจะขัดแย้งกันเอง (Master บังคับ 100% แต่อนุญาต Free-text เต็มรูปแบบ หรือ Master ไม่บังคับแต่ไม่อนุญาต Free-text เลย)
...
หัวข้อนี้รวบรวมข้อจำกัดชั่วคราว (Interim Limitation) จากหัวข้อ G ของแต่ละ P0 Decision ไว้เป็นภาพรวมเดียว **ข้อจำกัดชั่วคราวทุกข้อในตารางนี้ต้องได้รับการอนุมัติเป็นการเฉพาะจาก User ก่อนใช้งานจริง — ไม่ใช่การอนุมัติล่วงหน้าโดยอัตโนมัติ**
...
ตารางสรุปสำหรับ User ต่อไปนี้รวบรวม P0 Decision ทั้ง 9 รายการไว้ในที่เดียวเพื่อความสะดวกในการตัดสินใจ **ช่อง "User-selected option" ต้องว่างเปล่าเสมอ และ "สถานะ" ต้องเป็น `รอการตัดสินใจ` เสมอจนกว่า User จะกรอกด้วยตนเอง — ตารางนี้ไม่ใช่ทะเบียนที่อนุมัติแล้ว**

| ลำดับ | Decision ID | ประเด็นโดยย่อ | ทางเลือกที่แนะนำ (Non-binding) | User-selected option | เงื่อนไข | สถานะ | วันที่อนุมัติ | Knowledge ที่ต้องปรับปรุง |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | BDR-CUSTOMER-001 | Customer Master บังคับหรือไม่ | (Non-binding) Option C — Optional Master | | | รอการตัดสินใจ | | Topic 5 §6, §8 |
| 2 | BDR-CUSTOMER-002 | อนุญาต Free-text หรือไม่ | (Non-binding) Option B — Fallback เท่านั้น | | | รอการตัดสินใจ | | Topic 5 §6, §8 |
| 3 | BDR-EVIDENCE-001 | วิธีเก็บลายเซ็น | (Non-binding) Option D — Hybrid หลายวิธี | | | รอการตัดสินใจ | | Topic 5 §17, §18 |
| 4 | BDR-EVIDENCE-002 | ภาพถ่ายเอกสารแทนลายเซ็นได้หรือไม่ | (Non-binding) Option B — เทียบเท่าแบบมีเงื่อนไข | | | รอการตัดสินใจ | | Topic 5 §17, §34, §36 |
| 5 | BDR-EXTERNAL-001 | หลักฐานขั้นต่ำผู้ส่งภายนอก | (Non-binding) Option B — ขั้นต่ำที่ลดลงอย่างเป็นทางการ | | | รอการตัดสินใจ | | Topic 5 §21; Topic 6 |
| 6 | BDR-PREP-001 | อำนาจแก้ไขข้อมูลเตรียมสินค้าหลัง IN_TRANSIT | (Non-binding) Option C — Admin + ทบทวนย้อนหลังโดย Super Admin | | | รอการตัดสินใจ | | Topic 3 §14.2, §22; Topic 6 |
| 7 | BDR-PREP-004 | Admin สร้างบันทึกแก้ไขได้เองหรือไม่ | (Non-binding) Option C — แบ่งตามความรุนแรง | | | รอการตัดสินใจ | | Topic 3 §14.2, §22; Topic 5; Topic 6 |
| 8 | BDR-CORRECTION-001 | ฟิลด์ที่ใช้ Correction Action ได้ | (Non-binding) Option A — รับรองรายการ §20.1 | | | รอการตัดสินใจ | | Topic 3 §20.1; Topic 6 |
| 9 | BDR-RETURN-003 | Reopen ขณะ PENDING_RETURN | (Non-binding) Option A — บล็อกเด็ดขาด | | | รอการตัดสินใจ | | Topic 4 §22, §23; Topic 6 |

> [!warning]
> ตารางข้างต้นมี **9 แถว** ตรงกับจำนวน P0 ที่ยืนยันแล้ว คอลัมน์ "ทางเลือกที่แนะนำ" เป็นข้อเสนอเชิงวิเคราะห์แบบไม่ผูกมัดเสมอ (กำกับด้วยคำว่า "Non-binding" ทุกแถว) **ห้ามนำคอลัมน์นี้ไปใช้แทนการตัดสินใจของ User โดยเด็ดขาด**
...
> [!important]
> **ภารกิจนี้ไม่ดำเนินการ DISPATCH-BDR-P0-002** เอกสารฉบับนี้เพียงกำหนดขอบเขตของภารกิจถัดไปไว้ล่วงหน้าเท่านั้น
...
* **ห้ามทำเครื่องหมายประเด็น P0 ใดเป็น APPROVED โดยไม่มีคำสั่งชัดเจนจาก User** — ทุกช่อง "Selected option" ในหัวข้อ I ต้องว่างเปล่าจนกว่า User จะกรอกเอง
...
หัวข้อนี้สรุปผลกระทบของสถานะ P0 ปัจจุบัน (ยังไม่มีรายการใดอนุมัติ) ต่อความพร้อมของ MVP ตาม Release Gate ที่กำหนดไว้แล้วใน Topic 7 หัวข้อ 14 (GATE-01 ถึง GATE-11)

| MVP Feature Group | สถานะพร้อมออกแบบทางเทคนิคหรือไม่ | P0 ที่ยังบล็อกอยู่ |
| --- | --- | --- |
| MVP-01 (Identity, Roles, Access) | **พร้อม** — ไม่ถูกบล็อกโดย P0 ใด | ไม่มี |
| MVP-02 (Task Creation) | **ไม่พร้อม** | BDR-CUSTOMER-001, BDR-CUSTOMER-002 |
| MVP-03 (Preparation) | **ไม่พร้อม** | BDR-PREP-001, BDR-PREP-004 |
| MVP-04 (Assignment) | **ไม่พร้อมบางส่วน** (ขึ้นกับ Snapshot จาก MVP-02) | BDR-CUSTOMER-001, BDR-CUSTOMER-002 (โดยอ้อม) |
| MVP-05 (Delivery Start) | **พร้อม** | ไม่มี |
| MVP-06 (GPS Check-in) | **พร้อม** | ไม่มี |
| MVP-07 (Handover Evidence) | **ไม่พร้อม** | BDR-EVIDENCE-001, BDR-EVIDENCE-002 |
| MVP-08 (Quantity and Outcome) | **พร้อม** | ไม่มี |
| MVP-09 (Internal Normal Closure) | **ไม่พร้อมบางส่วน** (ขึ้นกับ MVP-07) | BDR-EVIDENCE-001, BDR-EVIDENCE-002 (โดยอ้อม) |
| MVP-10 (External Courier Closure) | **ไม่พร้อม** | BDR-EXTERNAL-001 |
| MVP-11 (Partial/Failed/Rescheduled) | **พร้อม** | ไม่มี |
| MVP-12 (Cancellation/Returned Goods) | **ไม่พร้อมบางส่วน** (ในจุดตัดกับ Reopen) | BDR-RETURN-003 |
| MVP-13 (Reopen and Reassignment) | **ไม่พร้อม** | BDR-RETURN-003 |
| MVP-14 (Emergency Override) | **พร้อม** | ไม่มี |
| MVP-15 (Super Admin Override Review) | **พร้อม** (P1 ค้างอยู่ ไม่บล็อก — ดูหัวข้อ 19) | ไม่มี (P0) |
| MVP-16 (Correction Action) | **ไม่พร้อม** | BDR-CORRECTION-001, BDR-PREP-004 |
| MVP-17 (Timeline and Audit Log) | **พร้อม** | ไม่มี |
| MVP-18 (Management/Audit Visibility) | **พร้อม** (P1 ค้างอยู่ ไม่บล็อก — ดูหัวข้อ 19) | ไม่มี (P0) |
| MVP-19 (Sensitive Data Governance) | **พร้อม** (P1 ค้างอยู่ ไม่บล็อก — ดูหัวข้อ 19) | ไม่มี (P0) |
| MVP-20 (Business Rule Validation) | **พร้อม** | ไม่มี |

> [!important]
> ณ ขณะที่จัดทำเอกสารฉบับนี้ (2026-07-19) **CP-01 (ก่อนเริ่ม Technical Architecture) ยังไม่ผ่าน** เนื่องจากยังไม่มี P0 Decision ใดได้รับการอนุมัติจาก User สถาปัตยกรรมทางเทคนิคของ MVP-02, MVP-03, MVP-07, MVP-09 (บางส่วน), MVP-10, MVP-12 (บางส่วน), MVP-13 และ MVP-16 ยังคงถูกบล็อกอยู่ ส่วน Feature Group ที่เหลือสามารถเริ่มงานสถาปัตยกรรมได้ทันทีตามขอบเขตที่อนุมัติแล้ว
