...
* ลายเซ็นลูกค้า (บังคับ) — BR-RECIPIENT-003
...
* **ความสามารถที่ไม่รวม**: Customer Master แบบบังคับ (เปิด — BDR-CUSTOMER-001), เลขอ้างอิงธุรกิจที่บังคับสากล (เปิด — BDR-TASK-001), Priority/Urgency บังคับ (เปิด — BDR-TASK-002)
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-CUSTOMER-001, BDR-CUSTOMER-002, BDR-CUSTOMER-003, BDR-TASK-001 — **MUST_DECIDE_BEFORE_MVP** (ดูหัวข้อ 16)
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-PREP-001 (อำนาจแก้ไขหลัง IN_TRANSIT) — **MUST_DECIDE_BEFORE_MVP**; BDR-PREP-002, BDR-PREP-003 — DECIDE_DURING_IMPLEMENTATION
...
* **ความสามารถที่ไม่รวม**: OTP, ลายเซ็นดิจิทัลแบบเต็มรูปแบบ, วิธีเก็บลายเซ็นทางเทคนิคที่เฉพาะเจาะจง (เปิด — BDR-EVIDENCE-001)
...
* **Rule ID ที่เกี่ยวข้อง**: BR-EVIDENCE-005 ถึง BR-EVIDENCE-009, BR-RECIPIENT-001 ถึง BR-RECIPIENT-007
* **Dependency**: ต้องเสร็จก่อน MVP-08 (Quantity and Attempt Outcome)
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-EVIDENCE-001, BDR-EVIDENCE-002 — **MUST_DECIDE_BEFORE_MVP**; BDR-EVIDENCE-003–011 — P1/P2
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-EXTERNAL-001 (ข้อกำหนดขั้นต่ำหลักฐาน) — **MUST_DECIDE_BEFORE_MVP**; BDR-EXTERNAL-002 ถึง 007 — P1/P2
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-RETURN-002 (หลักฐานบังคับคืนสินค้า), BDR-RETURN-007 (ยืนยันคืนสินค้าก่อนมอบหมาย Attempt ถัดไปหรือไม่) — P1; BDR-RETURN-003 (Reopen จาก CANCELLED ขณะ PENDING_RETURN) — **P0 / MUST_DECIDE_BEFORE_MVP** (บล็อกหลักที่ MVP-13 โดยตรง ดูหัวข้อ 16); BDR-RETURN-001, 004, 005, 006, 008, 009 — P2
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-REOPEN-001 — P2; BDR-REOPEN-003 (ชุดหลักฐานแยกต่างหากต่อรอบ) — P1; BDR-REOPEN-002 (จำนวนรอบสูงสุด) — P3/Deferred; BDR-RETURN-003 (Reopen จาก CANCELLED ขณะ PENDING_RETURN) — **MUST_DECIDE_BEFORE_MVP**
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-OVERRIDE-002 (Review Deadline), BDR-OVERRIDE-003 (ผลเมื่อปฏิเสธ) — **MUST_DECIDE_BEFORE_MVP** สำหรับการยอมรับ MVP-15 อย่างสมบูรณ์ แม้จะไม่บล็อกการเริ่มพัฒนา
...
* **ประเด็นที่ยังไม่ตัดสินใจ (Blocker)**: BDR-CORRECTION-001 (ฟิลด์ที่แก้ไขได้) — **MUST_DECIDE_BEFORE_MVP**; BDR-CORRECTION-002 ถึง 005 — P1/P2
...
| GATE-02 | P0 Business Decisions | ทุกประเด็น MUST_DECIDE_BEFORE_MVP / P0 ได้รับการอนุมัติ เลื่อนออกไป หรือยอมรับเป็นข้อจำกัดชั่วคราวอย่างชัดเจน | **Business Gate** |
...
> [!important]
> **ไม่มีรายการใดในทะเบียนนี้ถูกจัดเป็น APPROVED** เนื่องจากทุกรายการคือประเด็นที่ Topics 1–6 ระบุไว้อย่างชัดเจนว่ายังไม่ได้รับการยืนยัน รายการที่ได้รับการอนุมัติแล้วทั้งหมดถูกนำไปใช้เป็น Approved MVP Scope ในหัวข้อ [10](#10-approved-mvp-scope) แทน ไม่ปรากฏซ้ำในทะเบียนนี้
...
| 2 | BDR-CUSTOMER-001 | Customer/Destination | Customer Master Selection เป็นข้อบังคับหรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | PRODUCT_SCOPE, CUSTOMER_DESTINATION, DATA_REQUIREMENT | กำหนดรูปแบบข้อมูลลูกค้าทั้งหมดของ MVP-02 และ Data Model ปลายทาง | ออกแบบ Task Creation และ Snapshot ไม่ได้ | Product Owner / User | ก่อนเริ่มออกแบบ MVP-02 | Decide now | Topic 5 §6, §8 | BR-TASK-003 | P0 shortlist ข้อ 2; คู่กับ BDR-CUSTOMER-002 |
| 3 | BDR-CUSTOMER-002 | Customer/Destination | อนุญาตให้กรอกปลายทางแบบข้อความอิสระ (Free-text) หรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | PRODUCT_SCOPE, CUSTOMER_DESTINATION | เป็นทางเลือกคู่กับ Customer Master ที่กระทบ UX และ Data Model โดยตรง | ออกแบบฟอร์มสร้างงานไม่ได้ | Product Owner / User | ก่อนเริ่มออกแบบ MVP-02 | Decide now | Topic 5 §6, §8 | BR-TASK-003 | P0 shortlist ข้อ 2; คู่กับ BDR-CUSTOMER-001 |
...
| ลำดับ | Decision ID | หมวด | ประเด็นการตัดสินใจ | สถานะปัจจุบัน | Priority | ผลกระทบ | เหตุผลที่ต้องตัดสินใจ | ผลหากยังไม่ตัดสินใจ | ผู้อนุมัติ/เจ้าของ | จุดที่ต้องได้คำตอบ | แนวทางปัจจุบัน | Source Topic | Related Rule IDs | หมายเหตุ |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 7 | BDR-PREP-001 | Preparation | ผู้มีอำนาจแก้ไขข้อมูลการเตรียมสินค้าหลัง IN_TRANSIT (Admin เองได้ หรือจำกัดเฉพาะ Super Admin) | MUST_DECIDE_BEFORE_MVP | P0 | PERMISSION, WORKFLOW, DATA_REQUIREMENT | กำหนดขอบเขตอำนาจของ Correction/Exception Record ซึ่งเป็นกลไกบังคับของ MVP-03 | ไม่สามารถออกแบบสิทธิ์ของกระบวนการ Correction/Exception ได้ครบถ้วน | Product Owner / User (ร่วม Super Admin Governance) | ก่อนเริ่มพัฒนา MVP-03 | Decide now | Topic 3 §14.2; Topic 4 §27 | BR-PREP-007, BR-PREP-008 | P0 shortlist ข้อ 9 |
...
| ลำดับ | Decision ID | หมวด | ประเด็นการตัดสินใจ | สถานะปัจจุบัน | Priority | ผลกระทบ | เหตุผลที่ต้องตัดสินใจ | ผลหากยังไม่ตัดสินใจ | ผู้อนุมัติ/เจ้าของ | จุดที่ต้องได้คำตอบ | แนวทางปัจจุบัน | Source Topic | Related Rule IDs | หมายเหตุ |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 26 | BDR-EVIDENCE-001 | Evidence Quality | วิธีการทางเทคนิคที่แน่นอนของการเก็บลายเซ็น (วาดบนหน้าจอ, ถ่ายภาพเอกสาร หรือวิธีอื่น) | MUST_DECIDE_BEFORE_MVP | P0 | EVIDENCE, TECHNICAL_ARCHITECTURE, UX | ลายเซ็นเป็นหลักฐานบังคับปิดงาน (BR-RECIPIENT-003) — ต้องเลือกวิธีก่อนออกแบบ MVP-07 | ไม่สามารถออกแบบหน้าจอส่งมอบหรือ Data Model ของ MVP-07 ได้ | Product Owner / User (ร่วม UX และ Technical Architecture) | ก่อนเริ่มออกแบบ MVP-07 | Decide now | Topic 2 §5.13; Topic 5 §17, §39 กลุ่ม B | BR-EVIDENCE-*, BR-RECIPIENT-003 | P0 shortlist ข้อ 5; แม้เป็นเรื่องเทคนิคบางส่วน แต่ Method จำเป็นต่อ Business Acceptance Boundary |
| 27 | BDR-EVIDENCE-002 | Evidence Quality | ภาพถ่ายเอกสารที่ลงชื่อแล้วสามารถใช้แทนลายเซ็นบนหน้าจอได้หรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | EVIDENCE, WORKFLOW | เป็นทางเลือกเชิงนโยบายที่กระทบว่าหลักฐาน "ลายเซ็นครบถ้วน" หมายถึงอะไร | Validation ของ MVP-07/MVP-09 ไม่สามารถกำหนดเงื่อนไขผ่าน/ไม่ผ่านได้ชัดเจน | Product Owner / User | ก่อนเริ่มออกแบบ MVP-07 | Decide now | Topic 2 §5.13; Topic 5 §39 กลุ่ม B | BR-RECIPIENT-003 | P0 shortlist ข้อ 6; คู่กับ BDR-EVIDENCE-001 |
...
| ลำดับ | Decision ID | หมวด | ประเด็นการตัดสินใจ | สถานะปัจจุบัน | Priority | ผลกระทบ | เหตุผลที่ต้องตัดสินใจ | ผลหากยังไม่ตัดสินใจ | ผู้อนุมัติ/เจ้าของ | จุดที่ต้องได้คำตอบ | แนวทางปัจจุบัน | Source Topic | Related Rule IDs | หมายเหตุ |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 41 | BDR-EXTERNAL-001 | External Courier | ข้อกำหนดขั้นต่ำของหลักฐานที่ผู้ส่งสินค้าภายนอกต้องให้ได้ | MUST_DECIDE_BEFORE_MVP | P0 | EXTERNAL_COURIER, EVIDENCE, WORKFLOW | หลักฐานบังคับของงานภายนอกใช้ชุดเดียวกับงานภายในตามหลักการที่อนุมัติ (BR-EXTERNAL-005) แต่ผู้ส่งภายนอกอาจให้ข้อมูลได้ไม่ครบในทางปฏิบัติ — ต้องกำหนดขั้นต่ำที่ยอมรับได้จริง | ไม่สามารถออกแบบ Validation ของ MVP-10 ได้ครบถ้วน เสี่ยงต้องพึ่ง Emergency Override บ่อยเกินควร | Product Owner / User (ร่วม Admin Operations) | ก่อนเริ่มออกแบบ MVP-10 | Decide now | Topic 5 §21, §39 กลุ่ม F | BR-EXTERNAL-005, BR-EXTERNAL-009 | P0 shortlist ข้อ 7–8 |
...
| 49 | BDR-RETURN-003 | Reopen / Returned Goods | Task ที่ถูกยกเลิกพร้อม Returned-Goods = PENDING_RETURN สามารถถูก Reopen ได้หรือไม่ | MUST_DECIDE_BEFORE_MVP | P0 | REOPEN, CANCELLATION, RETURNED_GOODS, STATUS_TRANSITION | เป็นจุดตัดกันของสองกลไกอิสระ (Reopen และ Returned-Goods Status) ที่ Topic 4 ยังไม่ได้ให้ข้อสรุป | เสี่ยงต่อสถานะที่ขัดแย้งกันเอง (Task Active อีกครั้งขณะภาระคืนสินค้าเดิมค้างอยู่) หากไม่มีกฎชัดเจน | Product Owner / User (ร่วม Super Admin Governance) | ก่อนเริ่มพัฒนา MVP-13 | Decide now | Topic 4 §32 ข้อ 22 | BR-REOPEN-005, BR-RETURN-002 | P0 shortlist ข้อ 25 |
...
| 55 | BDR-CORRECTION-001 | Correction Action | ฟิลด์ข้อมูลผู้รับสินค้ารายการใดบ้างที่มีสิทธิ์ใช้ Correction Action ได้อย่างชัดเจน | MUST_DECIDE_BEFORE_MVP | P0 | CORRECTION_ACTION, RECIPIENT, GOVERNANCE | Correction Action เป็นกลไกบังคับของ MVP-16 แต่ยังไม่มีรายการฟิลด์ที่แก้ไขได้อย่างเป็นทางการ | เสี่ยงต่อการแก้ไขฟิลด์ที่ควรต้อง Reopen แทน หรือปฏิเสธการแก้ไขที่ควรทำได้ | Product Owner / User (ร่วม Admin Operations) | ก่อนเริ่มพัฒนา MVP-16 | Decide now | Topic 3 §27, §16 (recipient fields); Topic 5 §39 กลุ่ม A | BR-RECIPIENT-007 | P0 shortlist ข้อ 12 |
...
| 57 | BDR-PREP-004 | Preparation | Admin สามารถสร้างบันทึกแก้ไขสำหรับข้อมูลการเตรียมสินค้าหลังเริ่มจัดส่งได้เอง หรือจำกัดเฉพาะ Super Admin เท่านั้น | MUST_DECIDE_BEFORE_MVP | P0 | PERMISSION, CORRECTION_ACTION | เป็นการถามซ้ำเชิงลึกของ BDR-PREP-001 โดยเฉพาะขอบเขตของ Admin เทียบกับ Super Admin | เช่นเดียวกับ BDR-PREP-001 | Product Owner / User (ร่วม Super Admin Governance) | ก่อนเริ่มพัฒนา MVP-03/MVP-16 | Decide now | Topic 3 §27, §14.2; Topic 4 §32; Topic 5 §39 กลุ่ม G | BR-PREP-007, BR-PREP-008 | ควรตัดสินใจพร้อมกับ BDR-PREP-001 (ประเด็นเดียวกัน มุมมองต่างกัน) |
...
หัวข้อนี้ประเมินอย่างละเอียดว่าประเด็นใดใน Business Decision Register **เป็น Blocker ที่แท้จริงต่อการออกแบบ MVP** (P0) เทียบกับประเด็นที่ดูเร่งด่วนแต่แท้จริงแล้วเลื่อนได้ **ไม่ใช่ทุกประเด็นเปิดจะถูกจัดเป็น P0 โดยอัตโนมัติ** เอกสารฉบับนี้ประเมินอย่างน้อย 25 ประเด็นต่อไปนี้ตามที่ระบุไว้ในภารกิจ และ**ไม่เลือกคำตอบสุดท้ายให้กับประเด็นใด**

| # | ประเด็น | Decision ID | เป็น P0 หรือไม่ และเหตุผล | MVP Feature ที่ขึ้นกับประเด็นนี้ | ข้อจำกัดชั่วคราวที่อาจปลดล็อกการพัฒนาได้ (เป็นข้อเสนอ ไม่ใช่การตัดสินใจ) | ผู้ต้องอนุมัติ | จุดอนุมัติล่าสุดที่ยอมรับได้ |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 4 | วิธีเก็บลายเซ็นทางเทคนิค | BDR-EVIDENCE-001 | **ใช่ P0** — ลายเซ็นเป็นหลักฐานบังคับปิดงาน ไม่มีวิธีจึงออกแบบ MVP-07 ไม่ได้เลย | MVP-07, MVP-09, MVP-10 | ใช้วิธีวาดบนหน้าจอเป็นค่าเริ่มต้นชั่วคราวได้ หากอนุมัติเป็นการเฉพาะและพร้อมเปลี่ยนภายหลัง | Product Owner / User (ร่วม UX) | ก่อนเริ่มออกแบบ MVP-07 |
...
| --- | --- | --- | --- |
| CP-01 | ก่อนเริ่ม Technical Architecture | GATE-01 (Scope Approval) และ P0 Business Decisions (9 รายการในหัวข้อ 16) | Product Owner / User |
...
| 5 | ยังไม่ตัดสินใจว่าภาพเอกสารลงชื่อแทนลายเซ็นได้หรือไม่ | MUST_DECIDE_BEFORE_MVP | MUST_DECIDE_BEFORE_MVP (BDR-EVIDENCE-002) | P0 | MVP-07, MVP-09 | **ไม่ได้** (บล็อกการออกแบบ MVP-07) | Decide now — Product Owner / User | Topic 2, Topic 5 | บล็อก — ต้องรอ User ตัดสินใจก่อนออกแบบ MVP-07 |
| 6 | ข้อกำหนดขั้นต่ำหลักฐานผู้ส่งภายนอกยังไม่ได้ข้อสรุป | MUST_DECIDE_BEFORE_MVP | MUST_DECIDE_BEFORE_MVP (BDR-EXTERNAL-001) | P0 | MVP-10 | **ไม่ได้** (บล็อกการออกแบบ MVP-10) | Decide now — Product Owner / User | Topic 5 | บล็อก — ต้องรอ User ตัดสินใจก่อนออกแบบ MVP-10 |
...
| 17 | การ Reopen จาก CANCELLED ขณะ PENDING_RETURN ยังไม่ได้ข้อสรุป | MUST_DECIDE_BEFORE_MVP (BDR-RETURN-003) | MUST_DECIDE_BEFORE_MVP | P0 | MVP-12, MVP-13 | **ไม่ได้** (บล็อกการออกแบบ MVP-13) | Decide now — Product Owner / User | Topic 3, Topic 4 | บล็อก — ใช้ข้อจำกัดชั่วคราว "ห้าม Reopen จนกว่าจะยืนยันคืนสินค้า" ตามข้อเสนอในหัวข้อ 16 |
...
### 28.1 สรุปจำนวนตาม Decision Status (จากทะเบียน 93 รายการ)

| Decision Status | จำนวน |
| --- | --- |
| APPROVED (ปรากฏเฉพาะในหัวข้อ 10 ไม่ใช่ในทะเบียน) | 0 (ในทะเบียน) |
| MUST_DECIDE_BEFORE_MVP | 13 |
| DECIDE_DURING_IMPLEMENTATION | 52 |
| DEFERRED_PHASE_2 | 13 |
| TECHNICAL_DECISION | 5 |
| POLICY_REVIEW_REQUIRED | 9 |
| REJECTED_OR_OUT_OF_SCOPE | 1 (BDR-STATE-001 — กู้คืนจาก Topic 4 §32 ดูกลุ่ม L หัวข้อ 15) |
| SUPERSEDED | 0 |
| **รวม** | **93** |
...
> [!note]
> จำนวน P0 (Priority) = 9 รายการ ไม่เท่ากับจำนวน MUST_DECIDE_BEFORE_MVP (Status) = 13 รายการ เนื่องจากทั้งสองเป็นมิติที่แยกจากกันตามหลักการในหัวข้อ [6](#6-decision-priority-model) — มี 4 รายการที่เป็น MUST_DECIDE_BEFORE_MVP แต่มี Priority เป็น P1 เท่านั้น (BDR-RETURN-002, BDR-OVERRIDE-003, BDR-OVERRIDE-006, BDR-PRIVACY-001) เนื่องจากแม้ต้องตัดสินใจก่อนที่ Feature Group ที่เกี่ยวข้องจะเสร็จสมบูรณ์ แต่ไม่ได้บล็อกการเริ่มพัฒนา MVP โดยรวม ในทางกลับกัน BDR-STATE-001 มี Priority เพียง P3 แต่มี Decision Status เป็น REJECTED_OR_OUT_OF_SCOPE ซึ่งสาธิตว่า Priority ต่ำไม่ได้แปลว่าประเด็นนั้นยังคง "เปิด" อยู่เสมอ — บางประเด็นอาจถูกปิดด้วยการปฏิเสธ ไม่ใช่การอนุมัติ

### 28.3 สถานะความพร้อมของ MVP Feature Group

| กลุ่ม | Feature Group | สถานะความพร้อม |
| --- | --- | --- |
| พร้อมออกแบบทันที (ไม่มี P0 Blocker) | MVP-01, MVP-05, MVP-06, MVP-08, MVP-09, MVP-11, MVP-17, MVP-20 | **พร้อม** |
| รอ P0 Decision ก่อนเริ่มออกแบบ | MVP-02 (BDR-CUSTOMER-001/002), MVP-07 (BDR-EVIDENCE-001/002), MVP-10 (BDR-EXTERNAL-001), MVP-03 (BDR-PREP-001/004), MVP-16 (BDR-CORRECTION-001), MVP-13 (BDR-RETURN-003) | **บล็อกบางส่วน — รอ P0** |
| พัฒนาได้แต่ยังไม่ผ่านเกณฑ์ยอมรับเต็มรูปแบบ | MVP-04, MVP-12, MVP-14, MVP-15, MVP-18, MVP-19 | **พัฒนาได้ — รอปิด P1/P2 ก่อนยอมรับ** |

### 28.4 ข้อสรุป

* MVP มีขอบเขตที่ล็อกแล้วชัดเจน (หัวข้อ 10–11) และ Feature Group จำนวน 8 จาก 20 กลุ่มพร้อมเริ่มออกแบบทันทีโดยไม่มี P0 Blocker
* Feature Group จำนวน 6 กลุ่มต้องรอการตัดสินใจ P0 จาก User ก่อนเริ่มออกแบบ (ดูหัวข้อ 16)
* Feature Group ที่เหลือ 6 กลุ่มพัฒนาได้ทันทีแต่ต้องปิดประเด็น P1/P2 ก่อนผ่านเกณฑ์ยอมรับ
* ไม่มีประเด็นใดในทะเบียนถูกจัดเป็น APPROVED หรือ SUPERSEDED เนื่องจากทะเบียนรวบรวมเฉพาะประเด็นที่ยังไม่ได้รับการยืนยันจาก Topics 1–6 เท่านั้น มีเพียง 1 รายการ (BDR-STATE-001) ที่จัดเป็น REJECTED_OR_OUT_OF_SCOPE เนื่องจากขัดกับโมเดล Main Task Status ที่ล็อกไว้แล้วโดยตรง
