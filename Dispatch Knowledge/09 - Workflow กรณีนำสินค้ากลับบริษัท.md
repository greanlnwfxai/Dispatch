...
หากหลักฐานหรือข้อมูลจากผู้ส่งภายนอกไม่ครบ Admin อาจบันทึกเฉพาะข้อมูลที่ได้รับจริงพร้อมข้อจำกัดของข้อมูลนั้น โดยต้องแยก Physical Courier Identity ออกจาก Admin System Actor เสมอ การบันทึกข้อมูลบางส่วนไม่ทำให้ formal confirmation requirements ผ่านโดยอัตโนมัติ Admin ต้องใช้เฉพาะ data หรือ evidence requirements ที่ได้รับอนุมัติแล้ว และ exact mandatory return-evidence set ยังเปิดอยู่ภายใต้ BDR-RETURN-002 / BDR-RETURN-009 Topic 9 ไม่ลดหรือสร้าง requirement ใหม่เพื่อให้ confirmation ผ่าน
...
* เพิ่มรูปถ่ายเมื่อ required by existing rule หรือเมื่อเกี่ยวข้องกับ discrepancy
...
* รายละเอียด exact mandatory return-evidence set ยังเป็น Open Business Decision โดยเฉพาะ BDR-RETURN-002 และ BDR-RETURN-009
...
* Task และ Attempt ที่เกี่ยวข้อง
* เหตุผลที่เกิดภาระคืนสินค้า
* จำนวนคืนจริง หรือข้อเท็จจริงว่าจำนวนไม่ครบ/ไม่ทราบพร้อมหมายเหตุ
* สภาพสินค้าคืน
* ผู้ที่นำสินค้ากลับ เมื่อทราบ
* รายงาน Stock discrepancy เมื่อมี
* หลักฐานประกอบเมื่อเกี่ยวข้อง

> [!note]
> รายการข้อมูลธุรกิจข้างต้นเป็นข้อกำหนดที่ได้รับอนุมัติในระดับข้อมูลและการตรวจสอบย้อนกลับเท่านั้น ไม่ได้แก้ไขหรือแทนที่การตัดสินใจเรื่อง exact mandatory return-evidence set ซึ่งยังเปิดอยู่ภายใต้ BDR-RETURN-002 และ BDR-RETURN-009
...
| Mandatory return evidence set | Admin / Product Owner | Exact return-evidence requirement not yet approved | Apply only data or evidence requirements explicitly approved by existing BR/VR rules; do not define or enforce a new exact mandatory return-evidence set | Escalate to BDR decision; do not infer approval from operational guidance | BR-RETURN-006, VR-OPEN-001, BDR-RETURN-002, BDR-RETURN-009 | Open |
...
| ผู้ส่งภายนอกคืนสินค้าแต่ข้อมูลไม่ครบ | Admin อาจบันทึกข้อมูลที่ได้รับจริงพร้อมข้อจำกัด แยก courier identity จาก Admin actor; partial information does not automatically satisfy formal confirmation requirements; exact mandatory evidence remains open and Topic 9 must not lower or invent requirements to permit confirmation |
...
เอกสารนี้ไม่แก้ไขหรือปิด Open Business Decisions ต่อไปนี้

* **BDR-RETURN-002** — ข้อกำหนดหลักฐานที่บังคับสำหรับการคืนสินค้า
