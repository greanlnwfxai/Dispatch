...
> [!important] อนุมัติ 2026-07-20 — BDR-CUSTOMER-001 Option C, BDR-CUSTOMER-002 Option B
> ผู้สร้างงานต้องค้นหาและเลือก Customer/Destination จาก Customer Master เป็นทางเลือกแรกเสมอ หากไม่มี Master ที่เหมาะสม หรือปลายทางเป็นแบบเฉพาะกิจ จึงกรอกปลายทางแบบ Free-text ได้ ทุก Task ต้องบันทึกแหล่งที่มาของปลายทางเป็น **MASTER** หรือ **FREE_TEXT** เสมอ และต้องมี Historical Destination Snapshot ที่ตรึงค่าปลายทาง ณ เวลาใช้งานจริงไว้ (ดู [[05 - ข้อมูล หลักฐาน และรายละเอียดที่ต้องจัดเก็บในแต่ละงาน]] หัวข้อ 8) การเปลี่ยนแปลง Customer Master ในภายหลังต้องไม่กระทบ Snapshot ของ Task เดิม Free-text ต้องไม่ถูกใช้สร้างหรือเชื่อมโยง Master Record โดยอัตโนมัติ สิทธิ์เพิ่ม/แก้ไข Customer Master เป็นประเด็นแยกต่างหากที่ยังไม่ได้รับการอนุมัติ
...
#### Workflow synchronization: Preparation Correction

> [!important]
> * ข้อมูลการเตรียมงานเดิม (Preparation data) ถูกล็อกทันทีที่ Task เข้าสถานะ **IN_TRANSIT**
> * เมื่อ Stock พบว่าข้อมูลที่เตรียมไว้ไม่ตรงกับข้อมูลจริง (discrepancy) Admin สามารถสร้าง **Correction/Exception Record** ได้ทันทีในทุกกรณี
> * ทุก Record ต้องถูกจัดประเภทเป็น **NORMAL** หรือ **MATERIAL**
> * ระดับ Materiality มีผลต่อ**ความเร่งด่วนและลำดับการตรวจสอบเท่านั้น** ไม่มีผลต่อสิทธิ์ในการสร้างบันทึกหรือกระบวนการพื้นฐาน
> * ทุก Record ต้องผ่านการตรวจสอบย้อนหลัง (retrospective review) โดย Super Admin เสมอ ไม่ว่าจะเป็น NORMAL หรือ MATERIAL
> * **Preparation Correction เป็นกลไกคนละแบบกับ Emergency Override** — Preparation Correction ใช้แก้ไขข้อมูลการเตรียมงานที่ถูกล็อกแล้ว ไม่ใช่กลไกสำหรับข้ามเงื่อนไขบังคับก่อนปิดงาน
> * ข้อมูลเดิม Timeline และ Audit Log ต้องถูกเก็บรักษาไว้ครบถ้วนเสมอ ไม่ถูกเขียนทับหรือลบทิ้ง
...
> [!important] อนุมัติ 2026-07-20 — BDR-EVIDENCE-001 Option D, BDR-EVIDENCE-002 Option B
> ระบบรองรับ 3 วิธีเก็บลายเซ็นทางธุรกิจ: (1) วาดลายเซ็นบนหน้าจอ (2) อัปโหลดเอกสารที่ลงชื่อแล้ว (3) ถ่ายภาพใบเสร็จหรือเอกสารที่ลงชื่อแล้ว งานภายในใช้การวาดบนหน้าจอเป็นวิธีหลัก สำหรับงานผู้ส่งสินค้าภายนอก (External Courier) ที่ Admin บันทึกแทน การวาดลายเซ็นบนหน้าจอไม่เหมาะสมกับการปฏิบัติงานจริง (Operationally Unsuitable) เนื่องจากผู้ส่งภายนอกไม่มีสิทธิ์เข้าถึงระบบ Dispatch โดยตรง ดังนั้นหลักฐานเอกสารที่ลงชื่อแล้ว ไม่ว่าจะเป็นการอัปโหลดหรือถ่ายภาพ จึงเป็น **Fallback แบบมีเงื่อนไขตามบริบท (Conditional Contextual Fallback)** ที่ได้รับอนุมัติสำหรับกรณีงานผู้ส่งภายนอกโดยเฉพาะ **ไม่ใช่วิธีเทียบเท่าแบบไม่มีเงื่อนไข (Unconditional Full Equivalence)** กับการวาดลายเซ็นบนหน้าจอในทุกกรณี การใช้ Fallback นี้ต้องบันทึกเหตุผลกำกับเสมอ — Fallback ที่ครบเงื่อนไขถือเป็นการปิดงานปกติ (Normal Closure) ไม่ใช่ Emergency Override การเลือกไลบรารีหรือเทคโนโลยีเก็บลายเซ็นทางเทคนิคยังคงเป็นประเด็นแยกต่างหากที่จะถูกออกแบบในภายหลัง OTP ยังคงเป็นความสามารถที่พิจารณาในอนาคตแยกต่างหากจากลายเซ็น
...
> [!important] อนุมัติ 2026-07-20 — BDR-EXTERNAL-001 Option B
> ข้อกำหนดหลักฐานรวมสำหรับการปิดงานปกติของงานภายนอกยังคงเป็นชุดเดียวกับงานภายในไม่ลดลง แต่แบ่งความรับผิดชอบของแหล่งหลักฐานชัดเจน: **STEP ต้องบันทึกก่อนส่งมอบให้ผู้ส่งภายนอก** — รูปหลังโหลด/ก่อนส่งมอบ, จำนวนที่จัดส่ง, ข้อมูลปลายทาง, ชื่อและเบอร์โทรผู้รับที่บันทึกไว้แล้ว, ตัวตนผู้ให้บริการ/ผู้ส่งภายนอก **ผู้ส่งสินค้าภายนอกต้องส่งกลับให้ Admin อย่างน้อย** — หลักฐานถึงปลายทางที่ต้องระบุ**พิกัดตำแหน่ง (ละติจูด/ลองจิจูด) และวันเวลาที่ถึงปลายทาง โดยเชื่อมโยงกับ Delivery Attempt** นั้นโดยตรง, รูปหลักฐานการส่งมอบอย่างน้อย 1 รูป, ชื่อผู้รับจริง, หลักฐานลายเซ็นที่ยอมรับได้ตามหัวข้อ 5.13, จำนวนที่ส่งมอบจริง ขาดหลักฐานขั้นต่ำนี้ยังคงบล็อกการปิดงานปกติเช่นเดิม เส้นทางเดียวที่เหลือคือ Emergency Override
...
> [!important]
> ข้อผิดพลาดของ **ชื่อผู้รับสินค้า หมายเลขโทรศัพท์ผู้รับสินค้า แผนก หรือตำแหน่งของผู้รับสินค้า** ให้แก้ไขผ่าน **Recipient Correction Action** ซึ่งเป็นกลไกแก้ไขข้อมูลผู้รับที่แยกต่างหาก **โดยไม่ต้องเปิดงานกลับมาแก้ไข (Reopen)** ส่วนความคลาดเคลื่อนของ**จำนวนที่ส่งมอบ หลักฐาน (Evidence) ผลการจัดส่ง (Delivery Outcome) ข้อมูล Delivery Attempt หรือสถานะของ Task** ต้องดำเนินการผ่านกระบวนการเปิดงานกลับมาแก้ไข (Reopen) หรือกลไกกำกับดูแลที่เหมาะสมอื่น (เช่น Preparation Correction ตามหัวข้อ [5.4](#54-เบิกสินค้าจาก-stock) หรือ Emergency Override) เท่านั้น ห้ามใช้ Recipient Correction Action แก้ไขข้อมูลกลุ่มนี้
...
ตัวอย่างเหตุผลของการเปิดงานกลับมาแก้ไข ได้แก่ เอกสารที่ลงชื่อแล้วขาดหาย, จำนวนที่ส่งมอบไม่ถูกต้อง, เลือกผลลัพธ์ผิดพลาด, รูปภาพขาดหาย, วันหรือเวลาจัดส่งไม่ถูกต้อง, ไม่ได้บันทึกสินค้าที่ถูกส่งคืน, ข้อพิพาทจากลูกค้า, หรือพบข้อผิดพลาดจากการสุ่มตรวจสอบของ Admin

> [!warning]
> ห้ามแก้ไขข้อมูลประวัติที่เสร็จสมบูรณ์แล้วโดยไม่มีการควบคุมหรือไม่มีการบันทึกร่องรอย

> [!warning] เงื่อนไขก่อนเปิดงานกลับมาแก้ไข — งานที่ถูกยกเลิกและรอรับคืนสินค้า
> * งานที่อยู่ในสถานะ **CANCELLED** ควบคู่กับ **PENDING_RETURN** ต้อง**บล็อกการเปิดงานกลับมาแก้ไข (Reopen) สำหรับทั้ง Admin และ Super Admin**
> * ต้องเกิด **RETURN_CONFIRMED** ก่อนเสมอ จึงจะพิจารณาการเปิดงานกลับมาแก้ไขได้
> * การยืนยันรับคืน (RETURN_CONFIRMED) **ไม่ได้เปิดงานกลับมาแก้ไขโดยอัตโนมัติ** — การเปิดงานกลับมาแก้ไขยังคงเป็นการกระทำแยกต่างหากที่นำสถานะกลับไปที่ **ASSIGNED**
> * ประวัติการยกเลิก (Cancellation), Delivery Attempt และการรับคืนสินค้าต้องถูกเก็บรักษาไว้ครบถ้วน
> * **ไม่มีข้อยกเว้นให้ Super Admin ข้าม (bypass)** เงื่อนไขนี้ได้ไม่ว่ากรณีใด
...
* การเลือกเทคโนโลยีหรือไลบรารีที่ใช้ Implement การเก็บลายเซ็นลูกค้าทางเทคนิค — วิธีการทางธุรกิจทั้ง 3 วิธี (วาดลายเซ็นบนหน้าจอ, อัปโหลดเอกสารที่ลงชื่อแล้ว, ถ่ายภาพเอกสารที่ลงชื่อแล้ว) **ได้รับการอนุมัติแล้ว** ตามหัวข้อ [5.13](#513-บันทึกหลักฐานการส่งมอบ) มีเพียงรายละเอียดการออกแบบทางเทคนิคของการ Implement เท่านั้นที่ยังไม่ได้กำหนด
...
5. การเลือกเทคโนโลยีหรือไลบรารีสำหรับ Implement วิธีเก็บลายเซ็นลูกค้าทางเทคนิค — วิธีการทางธุรกิจทั้ง 3 วิธี (วาดลายเซ็นบนหน้าจอ, อัปโหลดเอกสารที่ลงชื่อแล้ว, ถ่ายภาพเอกสารที่ลงชื่อแล้ว) ได้รับการอนุมัติแล้ว ไม่ใช่ประเด็นที่ต้องยืนยันเพิ่มเติม
