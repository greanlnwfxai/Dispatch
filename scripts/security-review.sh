...
echo ""
echo "  [ ] Auth — no login/JWT implementation exists yet (AUTH-001 is a future milestone)"
echo "  [ ] RBAC — no role checks exist yet (no business endpoints in this milestone)"
echo "  [ ] Data privacy — GET /health exposes no PII, no secrets, only status+service"
echo "  [ ] Token security — no tokens are issued, stored, or logged anywhere yet"
